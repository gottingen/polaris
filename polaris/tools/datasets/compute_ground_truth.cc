...
std::vector<std::vector<std::pair<uint32_t, float>>> processUnfilteredParts(const std::string &base_file,
    ...
    for (int p = 0; p < num_parts; p++) {
        ...
        polaris::aligned_free(base_data);
    }
    return res;
};
...
int aux_main(const std::string &base_file, const std::string &query_file, const std::string &gt_file, size_t k,
    ...
    polaris::aligned_free(query_data);
...
namespace polaris {
    ...
    void run_compute_ground_truth_cli();

    void setup_compute_ground_truth_cli(collie::App *app) {


        app->add_option("--data_type", data_type, "data type <int8/uint8/float>")->required();
        app->add_option("--dist_fn", dist_fn, "distance function <l2/mips/cosine>")->required();
        app->add_option("--base_file", base_file, "File containing the base vectors in binary format")->required();
        app->add_option("--query_file", query_file, "File containing the query vectors in binary format")->required();
        app->add_option("--gt_file", gt_file,
                        "File name for the writing ground truth in binary format, please don't append"
                        ".bin at end if no filter_label or filter_label_file is provided it will save"
                        "the file with '.bin' at end. else it will save the file as filename_label.bin")->required();
        app->add_option("--K", K, "Number of ground truth nearest neighbors to compute")->required();
        app->add_option("--tags_file", tags_file, "File containing the tags in binary format")->default_val(
                std::string());
        app->callback(run_compute_ground_truth_cli);
    }
    ...
    void run_compute_ground_truth_cli() {
        ...
        try {
            ...
            polaris::cerr << "Compute GT failed." << std::endl;
            exit(-1);
        }
    }
}  // namespace polaris