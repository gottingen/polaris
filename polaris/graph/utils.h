...
inline size_t get_file_size(const std::string &fname)
{
    ...
    {
        polaris::cerr << "Could not open file: " << fname << std::endl;
        return 0;
    }
}
                                        ...
                                        const size_t &rounded_dim, size_t offset = 0)
{
    if (data == nullptr)
    {
        polaris::cerr << "Memory was not allocated for " << data << " before calling the load function. Exiting..."
                      << std::endl;
        throw polaris::PolarisException("Null pointer passed to copy_aligned_data_from_file function", -1, __PRETTY_FUNCTION__,
                                    __FILE__, __LINE__);
    }
    std::ifstream reader;
...
{
    if (!in.is_open())
        throw polaris::PolarisException("Index file size check called on unopened file stream", -1, __PRETTY_FUNCTION__, __FILE__,
                                    __LINE__);
    in.seekg(0, in.end);
