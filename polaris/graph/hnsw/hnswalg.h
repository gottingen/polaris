...
//                    if (candidate_id == 0) continue;
        ...
        search_base_layer_st(tableint ep_id, const void *data_point, size_t ef,
            ...
            if ((!has_deletions || !isMarkedDeleted(ep_id)) &&
                (!condition->is_in_blacklist(getExternalLabel(ep_id)))) {
                dist_t dist = fstdistfunc_(data_point, getDataByInternalId(ep_id), dist_func_param_);
                lowerBound = dist;
                top_candidates.emplace(dist, ep_id);
                    ...
                    if (!(visited_array[candidate_id] == visited_array_tag)) {
                        ...
                        if (top_candidates.size() < ef || lowerBound > dist) {
                            ...
                            if ((!has_deletions || !isMarkedDeleted(candidate_id)) &&
                                (!condition->is_in_blacklist(getExternalLabel(candidate_id))))
...
//                    if (candidate_id == 0) continue;
        ...
        */
        ...
        collie::Status search(polaris::SearchContext &ctx) const override {
            ...
            tableint currObj = enterpoint_node_;
            dist_t curdist = fstdistfunc_(ctx.query, getDataByInternalId(enterpoint_node_), dist_func_param_);
            ...
            std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> top_candidates;
            if (num_deleted_) {
                top_candidates = search_base_layer_st<true, true>(
                        currObj, ctx.query, std::max(ef_, static_cast<size_t>(ctx.top_k)), ctx.search_condition);
            } else {
                top_candidates = search_base_layer_st<false, true>(
                        currObj, ctx.query, std::max(ef_, static_cast<size_t>(ctx.top_k)), ctx.search_condition);
            }

            while (top_candidates.size() > static_cast<size_t>(ctx.top_k)) {
                top_candidates.pop();
            }
            while (top_candidates.size() > 0) {
