...
template <typename data_t> std::string PQL2Distance<data_t>::get_pivot_data_filename(const std::string &prefix) const
{
    if (_num_chunks == 0)
    {
        throw polaris::PolarisException("Must set num_chunks before calling get_pivot_data_filename", -1, __PRETTY_FUNCTION__,
                                    __FILE__, __LINE__);
    }
...
template POLARIS_API class PQL2Distance<int8_t>;
template POLARIS_API class PQL2Distance<uint8_t>;
template POLARIS_API class PQL2Distance<float>;
