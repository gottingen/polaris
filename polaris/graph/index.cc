...
Index<T, TagT, LabelT>::Index(const IndexConfig &index_config, std::shared_ptr<AbstractDataStore<T>> data_store,
    ...
    : _dist_metric(index_config.metric), _dim(index_config.dimension), _max_points(index_config.max_points),
      ...
      _delete_set(new turbo::flat_hash_set<uint32_t>), _conc_consolidate(index_config.concurrent_consolidate)
{
    if (_dynamic_index && !_enable_tags)
    {
        throw PolarisException("ERROR: Dynamic Indexing must have tags enabled.", -1, __PRETTY_FUNCTION__, __FILE__, __LINE__);
    }
...
void Index<T, TagT, LabelT>::save(const char *filename, bool compact_before_save)
{
    ...
    else
    {
        if (!_data_compacted)
        {
            throw PolarisException("Index save for non-compacted index is not yet implemented", -1, __PRETTY_FUNCTION__, __FILE__,
                               __LINE__);
        }
    }
...
size_t Index<T, TagT, LabelT>::load_tags(const std::string tag_filename)
{
    if (_enable_tags && !collie::filesystem::exists(tag_filename))
    {
        ...
        throw polaris::PolarisException("Tag file " + tag_filename + " does not exist!", -1, __PRETTY_FUNCTION__, __FILE__,
                                    __LINE__);
    }
    if (!_enable_tags)
    {
...
int Index<T, TagT, LabelT>::_get_vector_by_tag(TagType &tag, DataType &vec)
{
    ...
    {
        throw PolarisException("Error: bad any cast while performing _get_vector_by_tags() " + std::string(e.what()), -1);
    }
    ...
    const std::vector<LabelT> &filter_labels, bool search_invocation)
{
    ...
    {
        throw PolarisException("ERROR: Clear scratch space before passing.", -1, __PRETTY_FUNCTION__, __FILE__, __LINE__);
    }
    ...
    for (auto id : init_ids)
    {
        if (id >= _max_points + _num_frozen_pts)
        {
            ...
            throw polaris::PolarisException(std::string("Wrong loc") + std::to_string(id), -1, __PRETTY_FUNCTION__, __FILE__,
                                        __LINE__);
        }
                                                        ...
                                                        uint32_t filteredLindex)
{
    ...
    {
        throw polaris::PolarisException("ERROR: non-empty pruned_list passed", -1, __PRETTY_FUNCTION__, __FILE__, __LINE__);
    }
...
#pragma omp parallel for schedule(dynamic, 2048)
    ...
    if (_nd > 0)
    {
        polaris::cout << "done. Link time: " << ((double)link_timer.elapsed() / (double)1000000) << "s" << std::endl;
    }
}
...
void Index<T, TagT, LabelT>::set_start_points(const T *data, size_t data_count)
{
    ...
    std::unique_lock<std::shared_timed_mutex> tl(_tag_lock);
    if (_nd > 0)
        throw PolarisException("Can not set starting point for a non-empty index", -1, __PRETTY_FUNCTION__, __FILE__, __LINE__);
...
void Index<T, TagT, LabelT>::build_with_data_populated(const std::vector<TagT> &tags)
{
    ...
    if (_nd < 1)
        throw PolarisException("Error: Trying to build an index with 0 points", -1, __PRETTY_FUNCTION__, __FILE__, __LINE__);
    ...
    polaris::cout << "Index built with degree: max:" << max << "  avg:" << (float)total / (float)(_nd + _num_frozen_pts)
...
void Index<T, TagT, LabelT>::build(const T *data, const size_t num_points_to_load, const std::vector<TagT> &tags)
{
    ...
    if (_pq_dist)
    {
        throw PolarisException("ERROR: DO not use this build interface with PQ distance", -1, __PRETTY_FUNCTION__, __FILE__,
                           __LINE__);
    }
...
void Index<T, TagT, LabelT>::build(const char *filename, const size_t num_points_to_load, const std::vector<TagT> &tags)
{
    ...
    if (filename == nullptr)
    {
        throw polaris::PolarisException("Can not build with an empty file", -1, __PRETTY_FUNCTION__, __FILE__, __LINE__);
    }
...
void Index<T, TagT, LabelT>::build(const char *filename, const size_t num_points_to_load, const char *tag_filename)
{
    ...
    if (_enable_tags)
    {
        ...
        {
            throw PolarisException("Tag filename is null, while _enable_tags is set", -1, __PRETTY_FUNCTION__, __FILE__, __LINE__);
        }
        else
        {
            ...
            {
                throw polaris::PolarisException(std::string("Tag file") + tag_filename + " does not exist", -1, __PRETTY_FUNCTION__,
                                            __FILE__, __LINE__);
            }
        }
    }
                                                 ...
                                                 bool use_filters, const std::string filter_label)
{
    ...
    {
        throw PolarisException("Error: bad any cast while performing _search_with_tags() " + std::string(e.what()), -1);
    }
    catch (const std::exception &e)
    {
        throw PolarisException("Error: " + std::string(e.what()), -1);
...
template <typename T, typename TagT, typename LabelT> void Index<T, TagT, LabelT>::generate_frozen_point()
{
    ...
    {
        throw PolarisException("More than one frozen point not supported in generate_frozen_point", -1, __PRETTY_FUNCTION__,
                           __FILE__, __LINE__);
    }

    if (_nd == 0)
    {
        throw PolarisException("ERROR: Can not pick a frozen point since nd=0", -1, __PRETTY_FUNCTION__, __FILE__, __LINE__);
    }
...
{
    if (!_enable_tags)
        throw polaris::PolarisException("Point tag array not instantiated", -1, __PRETTY_FUNCTION__, __FILE__, __LINE__);

    {
        ...
        {
            polaris::cerr << "Error: _location_to_tag.size (" << _location_to_tag.size() << ")  + _delete_set->size ("
                          << _delete_set->size() << ") != _nd(" << _nd << ") ";
            return consolidation_report(polaris::consolidation_report::status_code::INCONSISTENT_COUNT_ERROR, 0, 0, 0,
                                        0, 0, 0, 0);
        }

        if (_location_to_tag.size() != _tag_to_location.size())
        {
            throw polaris::PolarisException("_location_to_tag and _tag_to_location not of same size", -1, __PRETTY_FUNCTION__,
                                        __FILE__, __LINE__);
        }
    }
    ...
    {
        throw polaris::PolarisException("ERROR: start node has been deleted", -1, __PRETTY_FUNCTION__, __FILE__, __LINE__);
    }
    ...
    for (uint32_t old = 0; old < _max_points + _num_frozen_pts; ++old)
    {
            ...
            || (old >= _max_points && old < _max_points + _num_frozen_pts))
        {
            ...
            for (auto ngh_iter : _graph_store->get_neighbours((location_t)old))
            {
                ...
                {
                    ++num_dangling;
                    polaris::cerr << "Error in compact_data(). _final_graph[" << old << "] has neighbor " << ngh_iter
...
template <typename T, typename TagT, typename LabelT> size_t Index<T, TagT, LabelT>::release_location(int location)
{
    if (_empty_slots.is_in_set(location))
        throw PolarisException("Trying to release location, but location already in empty slots", -1, __PRETTY_FUNCTION__, __FILE__,
                           __LINE__);
...
void Index<T, TagT, LabelT>::lazy_delete(const std::vector<TagT> &tags, std::vector<TagT> &failed_tags)
{
    ...
    {
        throw PolarisException("failed_tags should be passed as an empty list", -1, __PRETTY_FUNCTION__, __FILE__, __LINE__);
    }
    std::shared_lock<std::shared_timed_mutex> ul(_update_lock);
    std::unique_lock<std::shared_timed_mutex> tl(_tag_lock);
    std::unique_lock<std::shared_timed_mutex> dl(_delete_lock);
...
void Index<T, TagT, LabelT>::_get_active_tags(TagRobinSet &active_tags)
{
    ...
    {
        throw PolarisException("Error: bad_any cast while performing _get_active_tags() " + std::string(e.what()), -1);
    }
    catch (const std::exception &e)
    {
        throw PolarisException("Error :" + std::string(e.what()), -1);
...
{ // use after build or load
    if (_dynamic_index)
    {
        throw polaris::PolarisException("Optimize_index_layout not implemented for dyanmic indices", -1, __PRETTY_FUNCTION__,
                                    __FILE__, __LINE__);
    }
...
// EXPORTS
template POLARIS_API class Index<float, int32_t, uint32_t>;
template POLARIS_API class Index<int8_t, int32_t, uint32_t>;
template POLARIS_API class Index<uint8_t, int32_t, uint32_t>;
template POLARIS_API class Index<float, uint32_t, uint32_t>;
template POLARIS_API class Index<int8_t, uint32_t, uint32_t>;
template POLARIS_API class Index<uint8_t, uint32_t, uint32_t>;
template POLARIS_API class Index<float, int64_t, uint32_t>;
template POLARIS_API class Index<int8_t, int64_t, uint32_t>;
template POLARIS_API class Index<uint8_t, int64_t, uint32_t>;
template POLARIS_API class Index<float, uint64_t, uint32_t>;
template POLARIS_API class Index<int8_t, uint64_t, uint32_t>;
template POLARIS_API class Index<uint8_t, uint64_t, uint32_t>;
template POLARIS_API class Index<float, tag_uint128, uint32_t>;
template POLARIS_API class Index<int8_t, tag_uint128, uint32_t>;
template POLARIS_API class Index<uint8_t, tag_uint128, uint32_t>;
// Label with short int 2 byte
template POLARIS_API class Index<float, int32_t, uint16_t>;
template POLARIS_API class Index<int8_t, int32_t, uint16_t>;
template POLARIS_API class Index<uint8_t, int32_t, uint16_t>;
template POLARIS_API class Index<float, uint32_t, uint16_t>;
template POLARIS_API class Index<int8_t, uint32_t, uint16_t>;
template POLARIS_API class Index<uint8_t, uint32_t, uint16_t>;
template POLARIS_API class Index<float, int64_t, uint16_t>;
template POLARIS_API class Index<int8_t, int64_t, uint16_t>;
template POLARIS_API class Index<uint8_t, int64_t, uint16_t>;
template POLARIS_API class Index<float, uint64_t, uint16_t>;
template POLARIS_API class Index<int8_t, uint64_t, uint16_t>;
template POLARIS_API class Index<uint8_t, uint64_t, uint16_t>;
template POLARIS_API class Index<float, tag_uint128, uint16_t>;
template POLARIS_API class Index<int8_t, tag_uint128, uint16_t>;
template POLARIS_API class Index<uint8_t, tag_uint128, uint16_t>;
