...
namespace polaris {
    ...
    std::string PQL2Distance<data_t>::get_quantized_vectors_filename(const std::string &prefix) const {
        if (_num_chunks == 0) {
            throw polaris::PolarisException("Must set num_chunks before calling get_quantized_vectors_filename", -1,
                                            __PRETTY_FUNCTION__, __FILE__, __LINE__);
        }
        return polaris::get_quantized_vectors_filename(prefix, _is_opq, (uint32_t) _num_chunks);
    }

    template<typename data_t>
    std::string PQL2Distance<data_t>::get_pivot_data_filename(const std::string &prefix) const {
        if (_num_chunks == 0) {
            throw polaris::PolarisException("Must set num_chunks before calling get_pivot_data_filename", -1,
                                            __PRETTY_FUNCTION__,
                                            __FILE__, __LINE__);
        }
        return polaris::get_pivot_data_filename(prefix, _is_opq, (uint32_t) _num_chunks);
    }

    template<typename data_t>
    std::string PQL2Distance<data_t>::get_rotation_matrix_suffix(const std::string &pq_pivots_filename) const {
        return polaris::get_rotation_matrix_suffix(pq_pivots_filename);
    }
    ...
    void PQL2Distance<data_t>::load_pivot_data(const std::string &pq_table_file, size_t num_chunks) {
        ...
        polaris::load_bin<float>(pq_table_file, _tables, nr, nc, file_offset_data[0]);
        ...
        polaris::load_bin<float>(pq_table_file, _centroid, nr, nc, file_offset_data[1]);
        ...
        polaris::load_bin<uint32_t>(pq_table_file, _chunk_offsets, nr, nc, file_offset_data[chunk_offsets_index]);
        ...
        polaris::cout << "Loaded PQ Pivots: #ctrs: " << NUM_PQ_CENTROIDS << ", #dims: " << this->_ndims
        ...
        if (this->_is_opq) {
            ...
            polaris::load_bin<float>(rotmat_file, _rotmat_tr, nr, nc);
    ...
    template<typename data_t>
    uint32_t PQL2Distance<data_t>::get_num_chunks() const {
    ...
    template<typename data_t>
    void
    PQL2Distance<data_t>::preprocessed_distance(PQScratch<data_t> &pq_scratch, const uint32_t n_ids, float *dists_out) {
    ...
    template<typename data_t>
    float PQL2Distance<data_t>::brute_force_distance(const float *query_vec, uint8_t *base_vec) {
        float res = 0;
    ...
    class PQL2Distance<int8_t>;

    template POLARIS_API
    class PQL2Distance<uint8_t>;

    template POLARIS_API
    class PQL2Distance<float>;
