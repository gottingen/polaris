                               ...
                               const std::vector<std::string> &param_list)
{
    ...
    if (param_list.size() >= 6)
    {
        ...
        if (compress_ratio > 0 && compress_ratio <= 1)
        {
            ...
            {
                polaris::cout << "Compress ratio:" << compress_ratio << " new #pq_chunks:" << chunks_by_cr << std::endl;
                 ...
                 const std::string &labels_to_medoids_file)
{
    ...
    for (const auto &id_shard : node_shard)
    {
        ...
        if (shard_nnbrs == 0)
        {
            polaris::cout << "WARNING: shard #" << shard_id << ", node_id " << node_id << " has 0 nbrs" << std::endl;
        }
                          ...
                          const std::string out_metadata_file)
{
    ...
    polaris::cout << "fraction of dense points with >= " << density << " labels = " << (float)dense_pts / (float)npts
                  << std::endl;
...
template <typename T, typename LabelT>
uint32_t optimize_beamwidth(std::unique_ptr<polaris::PQFlashIndex<T, LabelT>> &pFlashIndex, T *tuning_sample,
    ...
    if (reorder_data_file != std::string(""))
    {
        ...
        try
        {
            ...
            if (reorder_data_file_size != 8 + sizeof(float) * (size_t)npts_reorder_file * (size_t)ndims_reorder_file)
                throw PolarisException("Discrepancy in reorder data file size ", -1, __PRETTY_FUNCTION__, __FILE__, __LINE__);
        }
                     ...
                     const uint32_t Lf)
{
    ...
    {
        polaris::cout << "Correct usage of parameters is R (max degree)\n"
    ...
    std::string pq_pivots_path = collie::filesystem::exists(pq_pivots_path_base) ? pq_pivots_path_base + "_pq_pivots.bin"
...
template POLARIS_API uint32_t optimize_beamwidth<int8_t, uint32_t>(
    std::unique_ptr<polaris::PQFlashIndex<int8_t, uint32_t>> &pFlashIndex, int8_t *tuning_sample,
    uint64_t tuning_sample_num, uint64_t tuning_sample_aligned_dim, uint32_t L, uint32_t nthreads, uint32_t start_bw);
template POLARIS_API uint32_t optimize_beamwidth<uint8_t, uint32_t>(
    std::unique_ptr<polaris::PQFlashIndex<uint8_t, uint32_t>> &pFlashIndex, uint8_t *tuning_sample,
    uint64_t tuning_sample_num, uint64_t tuning_sample_aligned_dim, uint32_t L, uint32_t nthreads, uint32_t start_bw);
template POLARIS_API uint32_t optimize_beamwidth<float, uint32_t>(
    std::unique_ptr<polaris::PQFlashIndex<float, uint32_t>> &pFlashIndex, float *tuning_sample,
    uint64_t tuning_sample_num, uint64_t tuning_sample_aligned_dim, uint32_t L, uint32_t nthreads, uint32_t start_bw);

template POLARIS_API uint32_t optimize_beamwidth<int8_t, uint16_t>(
    std::unique_ptr<polaris::PQFlashIndex<int8_t, uint16_t>> &pFlashIndex, int8_t *tuning_sample,
    uint64_t tuning_sample_num, uint64_t tuning_sample_aligned_dim, uint32_t L, uint32_t nthreads, uint32_t start_bw);
template POLARIS_API uint32_t optimize_beamwidth<uint8_t, uint16_t>(
    std::unique_ptr<polaris::PQFlashIndex<uint8_t, uint16_t>> &pFlashIndex, uint8_t *tuning_sample,
    uint64_t tuning_sample_num, uint64_t tuning_sample_aligned_dim, uint32_t L, uint32_t nthreads, uint32_t start_bw);
template POLARIS_API uint32_t optimize_beamwidth<float, uint16_t>(
    std::unique_ptr<polaris::PQFlashIndex<float, uint16_t>> &pFlashIndex, float *tuning_sample,
    uint64_t tuning_sample_num, uint64_t tuning_sample_aligned_dim, uint32_t L, uint32_t nthreads, uint32_t start_bw);
