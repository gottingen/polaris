 ...
 ***************************************************/
    ...
    template<typename T>
    uint32_t optimize_beamwidth(std::unique_ptr<polaris::PQFlashIndex<T>> &pFlashIndex, T *tuning_sample,
...
// This code should only be available in the "build" DLL.
    ...
    int build_disk_index(const char *dataFilePath, const char *indexFilePath, const char *indexBuildParameters,
        ...
        if (!std::is_same<T, float>::value &&
            (compareMetric == polaris::MetricType::METRIC_INNER_PRODUCT ||
             compareMetric == polaris::MetricType::METRIC_COSINE)) {
            std::stringstream stream;
            stream << "Disk-index build currently only supports floating point data for Max "
                      "Inner Product Search/ cosine similarity. "
                   << std::endl;
...
// This code should only be available in the "build" DLL.
    ...
    template POLARIS_API uint32_t optimize_beamwidth<int8_t>(
            std::unique_ptr<polaris::PQFlashIndex<int8_t>> &pFlashIndex, int8_t *tuning_sample,
            uint64_t tuning_sample_num, uint64_t tuning_sample_aligned_dim, uint32_t L, uint32_t nthreads,
            uint32_t start_bw);

    template POLARIS_API uint32_t optimize_beamwidth<uint8_t>(
            std::unique_ptr<polaris::PQFlashIndex<uint8_t>> &pFlashIndex, uint8_t *tuning_sample,
            uint64_t tuning_sample_num, uint64_t tuning_sample_aligned_dim, uint32_t L, uint32_t nthreads,
            uint32_t start_bw);

    template POLARIS_API uint32_t optimize_beamwidth<float>(
            std::unique_ptr<polaris::PQFlashIndex<float>> &pFlashIndex, float *tuning_sample,
            uint64_t tuning_sample_num, uint64_t tuning_sample_aligned_dim, uint32_t L, uint32_t nthreads,
            uint32_t start_bw);
