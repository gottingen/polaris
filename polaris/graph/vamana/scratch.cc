...
namespace polaris {
    ...
    template<typename T>
    SSDQueryScratch<T>::SSDQueryScratch(size_t aligned_dim, size_t visited_reserve) {
    ...
    template<typename T>
    SSDThreadData<T>::SSDThreadData(size_t aligned_dim, size_t visited_reserve) : scratch(aligned_dim,
                                                                                          visited_reserve) {
    }
    ...
    PQScratch<T>::PQScratch(size_t graph_degree, size_t aligned_dim) {
        ...
        polaris::alloc_aligned((void **) &aligned_pqtable_dist_scratch, 256 * (size_t) MAX_PQ_CHUNKS * sizeof(float),
                               256);
    ...
    class InMemQueryScratch<int8_t>;

    template POLARIS_API
    class InMemQueryScratch<uint8_t>;

    template POLARIS_API
    class InMemQueryScratch<float>;

    template POLARIS_API
    class SSDQueryScratch<int8_t>;

    template POLARIS_API
    class SSDQueryScratch<uint8_t>;

    template POLARIS_API
    class SSDQueryScratch<float>;

    template POLARIS_API
    class PQScratch<int8_t>;

    template POLARIS_API
    class PQScratch<uint8_t>;

    template POLARIS_API
    class PQScratch<float>;

    template POLARIS_API
    class SSDThreadData<int8_t>;

    template POLARIS_API
    class SSDThreadData<uint8_t>;

    template POLARIS_API
    class SSDThreadData<float>;
