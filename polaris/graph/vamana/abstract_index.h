...
namespace polaris {
    ...
    */
    ...
    public:
        ...
        virtual collie::Status build(const std::string &filename, size_t num_points_to_load, const std::vector<vid_t> &tags) = 0;
        virtual collie::Status build(const std::string &filename, size_t num_points_to_load, const std::string&tag_filename) = 0;
        virtual collie::Status build(const void *data, size_t num_points_to_load, const std::vector<vid_t> &tags) = 0;
        virtual collie::Status build(const std::string &data_file, size_t num_points_to_load) = 0;


        virtual collie::Status save(const char *filename, bool compact_before_save) = 0;
        virtual collie::Status save(const std::string &filename) {
            return save(filename.c_str(), false);
        }

        //virtual collie::Status export_properties(const std::string &filename) const = 0;

        [[nodiscard]] virtual collie::Status load(const char *index_file, uint32_t num_threads, uint32_t search_l) = 0;

        virtual collie::Status search(SearchContext &search_context) = 0;

        // insert point for unfiltered index build. do not use with filtered index
        virtual collie::Status insert_point(const void *point, const vid_t tag) = 0;

        virtual collie::Status get_vector(vid_t tag, void *vec) const= 0;

        virtual size_t size() const = 0;

        // delete point with tag, or return -1 if point can not be deleted
        virtual collie::Status lazy_delete(const vid_t &tag) = 0;

        // batch delete tags and populates failed tags if unabke to delete given tags.
        virtual collie::Status lazy_delete(const std::vector<vid_t> &tags, std::vector<vid_t> &failed_tags) = 0;
        ...
        virtual collie::Status get_vector_by_tag(vid_t &tag, void *vec) = 0;
    };
} // namespace polaris
