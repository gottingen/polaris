...
namespace polaris {
    ...
    PQDataStore<data_t>::PQDataStore(size_t dim, location_t num_points, size_t num_pq_chunks,
        ...
        if (num_pq_chunks > dim) {
            throw polaris::PolarisException("ERROR: num_pq_chunks > dim", -1, __PRETTY_FUNCTION__, __FILE__, __LINE__);
        }
    ...
    void PQDataStore<data_t>::prefetch_vector(const location_t loc) {
        ...
        polaris::prefetch_vector((const char *) ptr, _num_chunks * sizeof(data_t));
    }
    ...
    template<typename data_t>
    void
    PQDataStore<data_t>::copy_vectors(const location_t from_loc, const location_t to_loc, const location_t num_points) {
        // REFACTOR TODO: Is the number of bytes correct?
        memcpy(_quantized_data + to_loc * _num_chunks, _quantized_data + from_loc * _num_chunks,
               _num_chunks * num_points);
    }
...
// function should also do the alignment.
    ...
    void PQDataStore<data_t>::preprocess_query(const data_t *aligned_query, AbstractScratch<data_t> *scratch) const {
        if (scratch == nullptr) {
            throw polaris::PolarisException("Scratch space is null", -1);
        }

        PQScratch<data_t> *pq_scratch = scratch->pq_scratch();

        if (pq_scratch == nullptr) {
            throw polaris::PolarisException("PQScratch space has not been set in the scratch object.", -1);
        }
    ...
    template<typename data_t>
    float PQDataStore<data_t>::get_distance(const location_t loc1, const location_t loc2) const {
        throw std::logic_error("Not implemented yet");
    }
    ...
    void PQDataStore<data_t>::get_distance(const data_t *preprocessed_query, const location_t *locations,
        ...
        if (pq_scratch == nullptr) {
            throw polaris::PolarisException("PQScratch not set in scratch space.", -1);
        }
        polaris::aggregate_coords(locations, location_count, _quantized_data, this->_num_chunks,
    ...
    void PQDataStore<data_t>::get_distance(const data_t *preprocessed_query, const std::vector<location_t> &ids,
        ...
        polaris::aggregate_coords(ids, _quantized_data, this->_num_chunks, pq_scratch->aligned_pq_coord_scratch);
    ...
    template<typename data_t>
    location_t PQDataStore<data_t>::load_impl(const std::string &file_prefix) {
        if (_quantized_data != nullptr) {
            aligned_free(_quantized_data);
        }
    ...
    template<typename data_t>
    location_t PQDataStore<data_t>::expand(const location_t new_size) {
        throw std::logic_error("Not implemented yet");
    }

    template<typename data_t>
    location_t PQDataStore<data_t>::shrink(const location_t new_size) {
        throw std::logic_error("Not implemented yet");
    }


    template POLARIS_API
    class PQDataStore<int8_t>;

    template POLARIS_API
    class PQDataStore<float>;

    template POLARIS_API
    class PQDataStore<uint8_t>;
