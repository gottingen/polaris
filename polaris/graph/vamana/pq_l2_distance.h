...
namespace polaris {
    ...
    class PQL2Distance : public QuantizedDistance<data_t> {
    public:
        ...
        ~PQL2Distance() override;

        bool is_opq() const override;

        std::string get_quantized_vectors_filename(const std::string &prefix) const override;

        std::string get_pivot_data_filename(const std::string &prefix) const override;

        std::string get_rotation_matrix_suffix(const std::string &pq_pivots_filename) const override;

        void load_pivot_data(const std::string &pq_table_file, size_t num_chunks) override;

        // Number of chunks in the PQ table. Depends on the compression level used.
        // Has to be < ndim
        uint32_t get_num_chunks() const override;
        ...
        // PQFlashIndex class.
        void preprocess_query(const data_t *aligned_query, uint32_t original_dim,
        ...
        //
        void preprocessed_distance(PQScratch<data_t> &pq_scratch, const uint32_t id_count,
        ...
        void preprocessed_distance(PQScratch<data_t> &pq_scratch, const uint32_t n_ids,
        ...
        float brute_force_distance(const float *query_vec, uint8_t *base_vec) override;
