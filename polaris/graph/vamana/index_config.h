...
namespace polaris {
    ...
    private:
        IndexConfig(DataStoreStrategy data_strategy, GraphStoreStrategy graph_strategy, MetricType metric, size_t dimension,
                    size_t max_points, size_t num_pq_chunks, size_t num_frozen_points, bool dynamic_index,
                    bool enable_tags,
                ...
                : data_strategy(data_strategy), graph_strategy(graph_strategy), metric(metric), dimension(dimension),
                  max_points(max_points), dynamic_index(dynamic_index), enable_tags(enable_tags),
                  pq_dist_build(pq_dist_build),
                  concurrent_consolidate(concurrent_consolidate), use_opq(use_opq), filtered_index(filtered_index),
                  num_pq_chunks(num_pq_chunks), num_frozen_pts(num_frozen_points), label_type(label_type),
                  tag_type(tag_type),
    ...
    public:
        ...
        IndexConfigBuilder &with_index_write_params(std::shared_ptr<IndexWriteParameters> index_write_params_ptr) {
            if (index_write_params_ptr == nullptr) {
                polaris::cout << "Passed, empty build_params while creating index config" << std::endl;
                return *this;
            }
        ...
        IndexConfigBuilder &with_index_search_params(std::shared_ptr<IndexSearchParams> search_params_ptr) {
            if (search_params_ptr == nullptr) {
                polaris::cout << "Passed, empty search_params while creating index config" << std::endl;
                return *this;
            }
        ...
        IndexConfig build() {
            ...
            // sanity check
            if (_dynamic_index && _num_frozen_pts == 0) {
                polaris::cout << "_num_frozen_pts passed as 0 for dynamic_index. Setting it to 1 for safety."
                              << std::endl;
                _num_frozen_pts = 1;
            }
