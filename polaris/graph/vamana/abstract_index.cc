...
namespace polaris {
    ...
    void
    AbstractIndex::build(const data_type *data, const size_t num_points_to_load, const std::vector<tag_type> &tags) {
    ...
    int AbstractIndex::lazy_delete(const tag_type &tag) {
        auto any_tag = std::any(tag);
    ...
    template<typename tag_type>
    void AbstractIndex::get_active_tags(turbo::flat_hash_set<tag_type> &active_tags) {
    ...
    int AbstractIndex::get_vector_by_tag(tag_type &tag, data_type *vec) {
        auto any_tag = std::any(tag);
...
// exports
    template POLARIS_API void AbstractIndex::build<float, int32_t>(const float *data, const size_t num_points_to_load,
                                                                   const std::vector<int32_t> &tags);

    template POLARIS_API void AbstractIndex::build<int8_t, int32_t>(const int8_t *data,
                                                                    const size_t num_points_to_load,
                                                                    const std::vector<int32_t> &tags);

    template POLARIS_API void AbstractIndex::build<uint8_t, int32_t>(const uint8_t *data,
                                                                     const size_t num_points_to_load,
                                                                     const std::vector<int32_t> &tags);

    template POLARIS_API void AbstractIndex::build<float, uint32_t>(const float *data,
                                                                    const size_t num_points_to_load,
                                                                    const std::vector<uint32_t> &tags);

    template POLARIS_API void AbstractIndex::build<int8_t, uint32_t>(const int8_t *data,
                                                                     const size_t num_points_to_load,
                                                                     const std::vector<uint32_t> &tags);

    template POLARIS_API void AbstractIndex::build<uint8_t, uint32_t>(const uint8_t *data,
                                                                      const size_t num_points_to_load,
                                                                      const std::vector<uint32_t> &tags);

    template POLARIS_API void AbstractIndex::build<float, int64_t>(const float *data, const size_t num_points_to_load,
                                                                   const std::vector<int64_t> &tags);

    template POLARIS_API void AbstractIndex::build<int8_t, int64_t>(const int8_t *data,
                                                                    const size_t num_points_to_load,
                                                                    const std::vector<int64_t> &tags);

    template POLARIS_API void AbstractIndex::build<uint8_t, int64_t>(const uint8_t *data,
                                                                     const size_t num_points_to_load,
                                                                     const std::vector<int64_t> &tags);

    template POLARIS_API void AbstractIndex::build<float, uint64_t>(const float *data,
                                                                    const size_t num_points_to_load,
                                                                    const std::vector<uint64_t> &tags);

    template POLARIS_API void AbstractIndex::build<int8_t, uint64_t>(const int8_t *data,
                                                                     const size_t num_points_to_load,
                                                                     const std::vector<uint64_t> &tags);

    template POLARIS_API void AbstractIndex::build<uint8_t, uint64_t>(const uint8_t *data,
                                                                      const size_t num_points_to_load,
                                                                      const std::vector<uint64_t> &tags);
    ...
    template POLARIS_API size_t AbstractIndex::search_with_tags<float, int32_t>(
    ...
    template POLARIS_API size_t AbstractIndex::search_with_tags<uint8_t, int32_t>(
    ...
    template POLARIS_API size_t AbstractIndex::search_with_tags<int8_t, int32_t>(
    ...
    template POLARIS_API size_t AbstractIndex::search_with_tags<float, uint32_t>(
            const float *query, const uint64_t K, const uint32_t L, uint32_t *tags, float *distances,
            std::vector<float *> &res_vectors, bool use_filters, const std::string filter_label);

    template POLARIS_API size_t AbstractIndex::search_with_tags<uint8_t, uint32_t>(
            const uint8_t *query, const uint64_t K, const uint32_t L, uint32_t *tags, float *distances,
            std::vector<uint8_t *> &res_vectors, bool use_filters, const std::string filter_label);

    template POLARIS_API size_t AbstractIndex::search_with_tags<int8_t, uint32_t>(
            const int8_t *query, const uint64_t K, const uint32_t L, uint32_t *tags, float *distances,
            std::vector<int8_t *> &res_vectors, bool use_filters, const std::string filter_label);

    template POLARIS_API size_t AbstractIndex::search_with_tags<float, int64_t>(
            const float *query, const uint64_t K, const uint32_t L, int64_t *tags, float *distances,
            std::vector<float *> &res_vectors, bool use_filters, const std::string filter_label);

    template POLARIS_API size_t AbstractIndex::search_with_tags<uint8_t, int64_t>(
            const uint8_t *query, const uint64_t K, const uint32_t L, int64_t *tags, float *distances,
            std::vector<uint8_t *> &res_vectors, bool use_filters, const std::string filter_label);

    template POLARIS_API size_t AbstractIndex::search_with_tags<int8_t, int64_t>(
            const int8_t *query, const uint64_t K, const uint32_t L, int64_t *tags, float *distances,
            std::vector<int8_t *> &res_vectors, bool use_filters, const std::string filter_label);

    template POLARIS_API size_t AbstractIndex::search_with_tags<float, uint64_t>(
            const float *query, const uint64_t K, const uint32_t L, uint64_t *tags, float *distances,
            std::vector<float *> &res_vectors, bool use_filters, const std::string filter_label);

    template POLARIS_API size_t AbstractIndex::search_with_tags<uint8_t, uint64_t>(
            const uint8_t *query, const uint64_t K, const uint32_t L, uint64_t *tags, float *distances,
            std::vector<uint8_t *> &res_vectors, bool use_filters, const std::string filter_label);

    template POLARIS_API size_t AbstractIndex::search_with_tags<int8_t, uint64_t>(
    ...
    template POLARIS_API int AbstractIndex::insert_point<float, int32_t>(const float *point, const int32_t tag);

    template POLARIS_API int AbstractIndex::insert_point<uint8_t, int32_t>(const uint8_t *point, const int32_t tag);

    template POLARIS_API int AbstractIndex::insert_point<int8_t, int32_t>(const int8_t *point, const int32_t tag);

    template POLARIS_API int AbstractIndex::insert_point<float, uint32_t>(const float *point, const uint32_t tag);

    template POLARIS_API int AbstractIndex::insert_point<uint8_t, uint32_t>(const uint8_t *point, const uint32_t tag);

    template POLARIS_API int AbstractIndex::insert_point<int8_t, uint32_t>(const int8_t *point, const uint32_t tag);

    template POLARIS_API int AbstractIndex::insert_point<float, int64_t>(const float *point, const int64_t tag);

    template POLARIS_API int AbstractIndex::insert_point<uint8_t, int64_t>(const uint8_t *point, const int64_t tag);

    template POLARIS_API int AbstractIndex::insert_point<int8_t, int64_t>(const int8_t *point, const int64_t tag);

    template POLARIS_API int AbstractIndex::insert_point<float, uint64_t>(const float *point, const uint64_t tag);

    template POLARIS_API int AbstractIndex::insert_point<uint8_t, uint64_t>(const uint8_t *point, const uint64_t tag);

    template POLARIS_API int AbstractIndex::insert_point<int8_t, uint64_t>(const int8_t *point, const uint64_t tag);

    template POLARIS_API int AbstractIndex::insert_point<float, int32_t, uint16_t>(
            const float *point, const int32_t tag, const std::vector<uint16_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<uint8_t, int32_t, uint16_t>(
            const uint8_t *point, const int32_t tag, const std::vector<uint16_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<int8_t, int32_t, uint16_t>(
            const int8_t *point, const int32_t tag, const std::vector<uint16_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<float, uint32_t, uint16_t>(
            const float *point, const uint32_t tag, const std::vector<uint16_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<uint8_t, uint32_t, uint16_t>(
            const uint8_t *point, const uint32_t tag, const std::vector<uint16_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<int8_t, uint32_t, uint16_t>(
            const int8_t *point, const uint32_t tag, const std::vector<uint16_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<float, int64_t, uint16_t>(
            const float *point, const int64_t tag, const std::vector<uint16_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<uint8_t, int64_t, uint16_t>(
            const uint8_t *point, const int64_t tag, const std::vector<uint16_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<int8_t, int64_t, uint16_t>(
            const int8_t *point, const int64_t tag, const std::vector<uint16_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<float, uint64_t, uint16_t>(
            const float *point, const uint64_t tag, const std::vector<uint16_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<uint8_t, uint64_t, uint16_t>(
            const uint8_t *point, const uint64_t tag, const std::vector<uint16_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<int8_t, uint64_t, uint16_t>(
            const int8_t *point, const uint64_t tag, const std::vector<uint16_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<float, int32_t, uint32_t>(
            const float *point, const int32_t tag, const std::vector<uint32_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<uint8_t, int32_t, uint32_t>(
            const uint8_t *point, const int32_t tag, const std::vector<uint32_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<int8_t, int32_t, uint32_t>(
            const int8_t *point, const int32_t tag, const std::vector<uint32_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<float, uint32_t, uint32_t>(
            const float *point, const uint32_t tag, const std::vector<uint32_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<uint8_t, uint32_t, uint32_t>(
            const uint8_t *point, const uint32_t tag, const std::vector<uint32_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<int8_t, uint32_t, uint32_t>(
            const int8_t *point, const uint32_t tag, const std::vector<uint32_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<float, int64_t, uint32_t>(
            const float *point, const int64_t tag, const std::vector<uint32_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<uint8_t, int64_t, uint32_t>(
            const uint8_t *point, const int64_t tag, const std::vector<uint32_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<int8_t, int64_t, uint32_t>(
            const int8_t *point, const int64_t tag, const std::vector<uint32_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<float, uint64_t, uint32_t>(
            const float *point, const uint64_t tag, const std::vector<uint32_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<uint8_t, uint64_t, uint32_t>(
            const uint8_t *point, const uint64_t tag, const std::vector<uint32_t> &labels);

    template POLARIS_API int AbstractIndex::insert_point<int8_t, uint64_t, uint32_t>(
            const int8_t *point, const uint64_t tag, const std::vector<uint32_t> &labels);

    template POLARIS_API int AbstractIndex::lazy_delete<int32_t>(const int32_t &tag);

    template POLARIS_API int AbstractIndex::lazy_delete<uint32_t>(const uint32_t &tag);

    template POLARIS_API int AbstractIndex::lazy_delete<int64_t>(const int64_t &tag);

    template POLARIS_API int AbstractIndex::lazy_delete<uint64_t>(const uint64_t &tag);

    template POLARIS_API void AbstractIndex::lazy_delete<int32_t>(const std::vector<int32_t> &tags,
                                                                  std::vector<int32_t> &failed_tags);

    template POLARIS_API void AbstractIndex::lazy_delete<uint32_t>(const std::vector<uint32_t> &tags,
                                                                   std::vector<uint32_t> &failed_tags);

    template POLARIS_API void AbstractIndex::lazy_delete<int64_t>(const std::vector<int64_t> &tags,
                                                                  std::vector<int64_t> &failed_tags);

    template POLARIS_API void AbstractIndex::lazy_delete<uint64_t>(const std::vector<uint64_t> &tags,
                                                                   std::vector<uint64_t> &failed_tags);

    template POLARIS_API void AbstractIndex::get_active_tags<int32_t>(turbo::flat_hash_set<int32_t> &active_tags);

    template POLARIS_API void AbstractIndex::get_active_tags<uint32_t>(turbo::flat_hash_set<uint32_t> &active_tags);

    template POLARIS_API void AbstractIndex::get_active_tags<int64_t>(turbo::flat_hash_set<int64_t> &active_tags);

    template POLARIS_API void AbstractIndex::get_active_tags<uint64_t>(turbo::flat_hash_set<uint64_t> &active_tags);
    ...
    template POLARIS_API int AbstractIndex::get_vector_by_tag<int32_t, float>(int32_t &tag, float *vec);

    template POLARIS_API int AbstractIndex::get_vector_by_tag<int32_t, uint8_t>(int32_t &tag, uint8_t *vec);

    template POLARIS_API int AbstractIndex::get_vector_by_tag<int32_t, int8_t>(int32_t &tag, int8_t *vec);

    template POLARIS_API int AbstractIndex::get_vector_by_tag<uint32_t, float>(uint32_t &tag, float *vec);

    template POLARIS_API int AbstractIndex::get_vector_by_tag<uint32_t, uint8_t>(uint32_t &tag, uint8_t *vec);

    template POLARIS_API int AbstractIndex::get_vector_by_tag<uint32_t, int8_t>(uint32_t &tag, int8_t *vec);

    template POLARIS_API int AbstractIndex::get_vector_by_tag<int64_t, float>(int64_t &tag, float *vec);

    template POLARIS_API int AbstractIndex::get_vector_by_tag<int64_t, uint8_t>(int64_t &tag, uint8_t *vec);

    template POLARIS_API int AbstractIndex::get_vector_by_tag<int64_t, int8_t>(int64_t &tag, int8_t *vec);

    template POLARIS_API int AbstractIndex::get_vector_by_tag<uint64_t, float>(uint64_t &tag, float *vec);

    template POLARIS_API int AbstractIndex::get_vector_by_tag<uint64_t, uint8_t>(uint64_t &tag, uint8_t *vec);

    template POLARIS_API int AbstractIndex::get_vector_by_tag<uint64_t, int8_t>(uint64_t &tag, int8_t *vec);
