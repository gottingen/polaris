...
public:
    ...
    bool open() {
        ...
        switch (dataType) {
            case DataTypeFloat:
                ...
                objectSpace = new polaris::ObjectSpaceRepository<float, double>(genuineDimension, typeid(float),
                                                                                distanceType);
                break;
            case DataTypeUint8:
                genuineDimension = ArrayFile<polaris::Object>::_fileHead.recordSize / sizeof(uint8_t);
                objectSpace = new polaris::ObjectSpaceRepository<unsigned char, int>(genuineDimension, typeid(uint8_t),
                                                                                     distanceType);
                break;
            case DataTypeFloat16:
                genuineDimension = ArrayFile<polaris::Object>::_fileHead.recordSize / sizeof(polaris::float16);
                objectSpace = new polaris::ObjectSpaceRepository<polaris::float16, float>(genuineDimension,
                                                                                          typeid(polaris::float16),
                                                                                          distanceType);
                break;
            default:
                stringstream msg;
    ...
    bool open(const std::string &file, DataType datat, polaris::MetricType distt, size_t pseudoDim) {
        dataType = datat;
        fileName = file;
        distanceType = distt;
    ...
    bool openMultipleStreams(const size_t nOfStreams) {
        ...
        if (!objectFiles.empty()) {
            std::cerr
                    << "ObjectFile::openMultipleStreams : already opened multiple streams. close and reopen. # of streams="
                    << nOfStreams << std::endl;
            closeMultipleStreams();
        }
...
bool StaticObjectFile<TYPE>::openMultipleStreams(const size_t nOfStreams) {
    ...
    if (!_objectFiles.empty()) {
        std::cerr << "StaticObjectFile : already opened multiple streams. close and reopen. # of streams=" << nOfStreams
                  << std::endl;
        closeMultipleStreams();
    }
    for (size_t i = 0; i < nOfStreams; i++) {
