...
void QBG::Optimizer::evaluate(string global, vector<vector<float>> &vectors, char clusteringType, string &ofile,
                              size_t &numberOfSubvectors, size_t &subvectorSize) {
    vector<vector<float>> residualVectors;
    {
        ...
        if (!global.empty()) {
            ...
            for (size_t cidx = 0; cidx < globalCentroid.size(); ++cidx) {
                for (auto mit = globalCentroid[cidx].members.begin();
                     mit != globalCentroid[cidx].members.end(); ++mit) {
    ...
    try {
        ...
        if (numberOfObjects == 0) {
            ...
            numberOfObjects = index.getQuantizer().objectList.size() - 1 < numberOfObjects ?
                              index.getQuantizer().objectList.size() - 1 : numberOfObjects;
        }
        ...
        if (index.getQuantizer().property.localCentroidLimit != 0 && numberOfClusters != 0 &&
            ...
            std::stringstream msg;
            msg << "optimize: # of clusters is already specified. " << index.getQuantizer().property.localCentroidLimit
                << ":" << numberOfClusters;
            POLARIS_THROW_EX(msg);
...
void QBG::Optimizer::optimize(vector<vector<float>> &vectors, vector<vector<float>> &globalCentroid, Matrix<float> &r,
                              vector<vector<polaris::Clustering::Cluster>> &localClusters, vector<double> &errors) {
    if (vectors.size() == 0) {
        std::stringstream msg;
    ...
    for (int step = seedNumberOfSteps - 1;; step--) {
        ...
        if (vsize <= 1) {
            ...
            msg << "# of partial vectors is too small, because # of vectors is too small or seedStep is too large."
                << " # of partial vectors=" << vsize << " # of vectors=" << vectors.size() << " seedStep=" << seedStep
                << std::endl;
            POLARIS_THROW_EX(msg);
        }
