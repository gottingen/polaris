...
namespace polaris {
    ...
    public:
        ...
        public:
            ...
            explicit ID(NodeID i) {
                setID(i);
            }
            ID(const ID &n) = default;
            ID &operator=(const ID &n) = default;
            ...
            void serialize(std::ofstream &os) { polaris::Serializer::write(os, id); }

            void deserialize(std::ifstream &is) { polaris::Serializer::read(is, id); }

            void serializeAsText(std::ofstream &os) { polaris::Serializer::writeAsText(os, id); }

            void deserializeAsText(std::ifstream &is) { polaris::Serializer::readAsText(is, id); }
        ...
        void setPivot(polaris::Object &f, ObjectSpace &os) {
            if (pivot == 0) {
                pivot = polaris::Object::allocate(os);
            }
            os.copy(getPivot(), f);
        }

        polaris::Object &getPivot() { return *pivot; }
    ...
    public:
        ...
        InternalNode(polaris::ObjectSpace *os = 0) : childrenSize(5) { initialize(); }
        ...
        void serialize(std::ofstream &os, ObjectSpace *objectspace = 0) {
            ...
            polaris::Serializer::write(os, childrenSize);
            for (size_t i = 0; i < childrenSize; i++) {
                getChildren()[i].serialize(os);
            }
            for (size_t i = 0; i < childrenSize - 1; i++) {
                polaris::Serializer::write(os, getBorders()[i]);
            }
        }
        ...
        void deserialize(std::ifstream &is, ObjectSpace *objectspace = 0) {
            ...
            polaris::Serializer::read(is, childrenSize);
            assert(children != 0);
            for (size_t i = 0; i < childrenSize; i++) {
            ...
            assert(borders != 0);
            for (size_t i = 0; i < childrenSize - 1; i++) {
                polaris::Serializer::read(is, getBorders()[i]);
            }
        }
        ...
        void serializeAsText(std::ofstream &os, ObjectSpace *objectspace = 0) {
            ...
            os << " ";
            polaris::Serializer::writeAsText(os, childrenSize);
            os << " ";
            for (size_t i = 0; i < childrenSize; i++) {
            ...
            for (size_t i = 0; i < childrenSize - 1; i++) {
                polaris::Serializer::writeAsText(os, getBorders()[i]);
                os << " ";
            }
        }
        ...
        void deserializeAsText(std::ifstream &is, ObjectSpace *objectspace = 0) {
            ...
            size_t csize;
            polaris::Serializer::readAsText(is, csize);
            assert(children != 0);
            ...
            assert(borders != 0);
            for (size_t i = 0; i < childrenSize - 1; i++) {
                polaris::Serializer::readAsText(is, getBorders()[i]);
            }
        }
    ...
    public:
        LeafNode(polaris::ObjectSpace *os = 0) {
            id = 0;
            id.setType(ID::Leaf);
            pivot = 0;
...
#else
            objectSize = 0;
            objectIDs = new polaris::ObjectDistance[LeafObjectsSizeMax];
#endif
        }
        ...
        polaris::ObjectDistance *getObjectIDs() { return objectIDs; }

        void serialize(std::ofstream &os, ObjectSpace *objectspace = 0) {
            Node::serialize(os);
#ifdef NGT_NODE_USE_VECTOR
            polaris::Serializer::write(os, objectIDs);
#else
            polaris::Serializer::write(os, objectSize);
...
#ifdef NGT_NODE_USE_VECTOR
            objectIDs.clear();
            polaris::Serializer::read(is, objectIDs);
#else
            assert(objectIDs != 0);
            polaris::Serializer::read(is, objectSize);
            for (int i = 0; i < objectSize; i++) {
        ...
        void serializeAsText(std::ofstream &os, ObjectSpace *objectspace = 0) {
            ...
            os << " ";
#ifdef NGT_NODE_USE_VECTOR
            polaris::Serializer::writeAsText(os, objectIDs);
#else
            polaris::Serializer::writeAsText(os, objectSize);
            for (int i = 0; i < objectSize; i++) {
                os << " ";
...
#ifdef NGT_NODE_USE_VECTOR
            objectIDs.clear();
            polaris::Serializer::readAsText(is, objectIDs);
#else
            assert(objectIDs != 0);
            polaris::Serializer::readAsText(is, objectSize);
            for (int i = 0; i < objectSize; i++) {
