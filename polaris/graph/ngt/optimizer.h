...
namespace polaris {
    ...
    public:
        ...
        class MeasuredValue {
        public:
            MeasuredValue() : keyValue(0.0), totalCount(0), meanAccuracy(0.0), meanTime(0.0), meanDistanceCount(0.0),
                              meanVisitCount(0.0) {}
        ...
        evaluate(std::istream &resultStream, std::string &type,
                 size_t &resultDataSize, size_t specifiedResultSize = 0, size_t groundTruthSize = 0,
                 bool recall = false) {
                                ...
                                if (gtf.size() >= 1 && gtf[0] == "# End of Search") {
                                    if (prevDataCount == 0) {
                                        ...
                                        if (prevDataCount != dataCount) {
                                            warn = true;
                                            std::cerr
                                                    << "Warning!: Result sizes are inconsistent! $prevDataCount:$dataCount"
                                                    << std::endl;
                                    ...
                                    if (gtf.size() >= 2 && (gtf[0] == "# Epsilon" || gtf[0] == "# Factor")) {
                                        ...
                                        if (recall == false && resultDataSize != dataCount) {
                                            std::cerr
                                                    ...
                                                    << std::endl;
                                            std::cerr
                                                    << "  Use $resultDataSize instead of $dataCount as the result size to compute accuracy. "
                                                    << std::endl;
                                            std::cerr << "    # of the actual search resultant objects=" << dataCount
                                                      << std::endl;
                                            std::cerr
                                                    << "    the specified # of search objects or # of the ground truth data="
                                        ...
                                        }
                                        {
                                            ...
                                            if (di != sumupValues.totalDistanceCount.end()) {
                                                ...
                                                sumupValues.totalDistanceCount.insert(
                                                        std::make_pair(key, distanceCount));
                                            }
                                        }
                                        {
        ...
        MeasuredValue
        measure(std::istream &queries, std::istream &gtStream, Command::SearchParameters &searchParameters,
                std::pair<float, float> accuracyRange, double margin) {
            ...
            calculateMeanValues(accuracies, accuracyRange.first, accuracyRange.second, size, distanceCount, visitCount,
                                time);
            if (distanceCount == 0) {
                std::stringstream msg;
        ...
        adjustBaseSearchEdgeSize(std::stringstream &queries, Command::SearchParameters &searchParameters,
                                 std::stringstream &gtStream, std::pair<float, float> accuracyRange,
                    ...
                    for (size_t baseStep = 16; baseStep != 1; baseStep /= 2) {
                        ...
                        for (size_t base = baseStart;; base += baseStep) {
                            ...
                            if (times.count(base) == 0) {
                                for (;;) {
                                    try {
                                        auto values = measure(queries, gtStream, searchParameters, accuracyRange,
                                                              margin);
                            ...
                            if (prevTime <= time) {
                                if (baseStep == minimumStep) {
                                    ...
                                    baseStart = static_cast<int>(minBase) - static_cast<int>(baseStep) <
                                                static_cast<int>(baseStart) ? baseStart : minBase - baseStep;
                                    break;
                                }
                            }
                            prevTime = time;
        ...
        std::pair<size_t, double>
        adjustRateSearchEdgeSize(std::stringstream &queries, Command::SearchParameters &searchParameters,
                                 std::stringstream &gtStream, std::pair<float, float> accuracyRange,
                        ...
                        for (size_t rate = rateStart; rate < 2000; rate += rateStep) {
                            ...
                            if (prevTime <= time) {
                                if (rateStep == minimumStep) {
                                    ...
                                    rateStart = static_cast<int>(minRate) - static_cast<int>(rateStep) <
                                                static_cast<int>(rateStart) ? rateStart : minRate - rateStep;
                                    break;
                                }
                            }
                            prevTime = time;
        ...
        adjustSearchEdgeSize(std::pair<float, float> baseAccuracyRange, std::pair<float, float> rateAccuracyRange,
                             size_t querySize, double epsilon, float margin = 0.2) {
        ...
        calculateMeanValues(std::vector<MeasuredValue> &accuracies, double accuracyRangeFrom, double accuracyRangeTo,
            ...
            }
            {
                ...
                for (size_t i = 0; i < distance.size() - 1; ++i) {
                    area += ((distance[i].first + distance[i + 1].first) *
                             (distance[i + 1].second - distance[i].second)) / 2.0;
                }
