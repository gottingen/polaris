        ...
        public:
            ...
            void parse(Args &args, const std::string epsilonDefault) {
                ...
                trial = args.getl("t", 1);
                {
                    ...
                    polaris::Common::tokenize(epsilon, tokens, ":");
                    if (tokens.size() >= 1) { beginOfEpsilon = endOfEpsilon = polaris::Common::strtod(tokens[0]); }
                    if (tokens.size() >= 2) { endOfEpsilon = polaris::Common::strtod(tokens[1]); }
                    if (tokens.size() >= 3) { stepOfEpsilon = polaris::Common::strtod(tokens[2]); }
                    step = 0;
                    if (tokens.size() >= 4) { step = polaris::Common::strtol(tokens[3]); }
                }
