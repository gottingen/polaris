...
template <typename T> SSDQueryScratch<T>::SSDQueryScratch(size_t aligned_dim, size_t visited_reserve)
{
    ...
    polaris::alloc_aligned((void **)&coord_scratch, coord_alloc_size, 256);
    polaris::alloc_aligned((void **)&sector_scratch, defaults::MAX_N_SECTOR_READS * defaults::SECTOR_LEN,
                           defaults::SECTOR_LEN);
    polaris::alloc_aligned((void **)&this->_aligned_query_T, aligned_dim * sizeof(T), 8 * sizeof(T));
...
{
    polaris::aligned_free((void *)coord_scratch);
    polaris::aligned_free((void *)sector_scratch);
    polaris::aligned_free((void *)this->_aligned_query_T);
...
{
    polaris::alloc_aligned((void **)&aligned_pq_coord_scratch,
                           (size_t)graph_degree * (size_t)MAX_PQ_CHUNKS * sizeof(uint8_t), 256);
    polaris::alloc_aligned((void **)&aligned_pqtable_dist_scratch, 256 * (size_t)MAX_PQ_CHUNKS * sizeof(float), 256);
    polaris::alloc_aligned((void **)&aligned_dist_scratch, (size_t)graph_degree * sizeof(float), 256);
    polaris::alloc_aligned((void **)&aligned_query_float, aligned_dim * sizeof(float), 8 * sizeof(float));
    polaris::alloc_aligned((void **)&rotated_query, aligned_dim * sizeof(float), 8 * sizeof(float));
...
{
    polaris::aligned_free((void *)aligned_pq_coord_scratch);
    polaris::aligned_free((void *)aligned_pqtable_dist_scratch);
    polaris::aligned_free((void *)aligned_dist_scratch);
    polaris::aligned_free((void *)aligned_query_float);
    polaris::aligned_free((void *)rotated_query);
}
...
template POLARIS_API class InMemQueryScratch<int8_t>;
template POLARIS_API class InMemQueryScratch<uint8_t>;
template POLARIS_API class InMemQueryScratch<float>;

template POLARIS_API class SSDQueryScratch<int8_t>;
template POLARIS_API class SSDQueryScratch<uint8_t>;
template POLARIS_API class SSDQueryScratch<float>;

template POLARIS_API class PQScratch<int8_t>;
template POLARIS_API class PQScratch<uint8_t>;
template POLARIS_API class PQScratch<float>;

template POLARIS_API class SSDThreadData<int8_t>;
template POLARIS_API class SSDThreadData<uint8_t>;
template POLARIS_API class SSDThreadData<float>;
