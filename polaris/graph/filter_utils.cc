...
{
    polaris::IndexWriteParameters label_index_build_parameters = polaris::IndexWriteParametersBuilder(L, R)
    ...
    polaris::cout.setstate(std::ios_base::failbit);
    for (const auto &lbl : all_labels)
    {
        ...
        polaris::get_bin_metadata(curr_label_input_data_path, number_of_label_points, dimension);

        polaris::Index<T> index(polaris::Metric::L2, dimension, number_of_label_points,
                                std::make_shared<polaris::IndexWriteParameters>(label_index_build_parameters), nullptr,
...
std::tuple<std::vector<std::vector<LabelT>>, turbo::flat_hash_set<LabelT>> parse_formatted_label_file(std::string label_file)
{
    std::vector<std::vector<LabelT>> pts_to_labels;
    turbo::flat_hash_set<LabelT> labels;
    ...
    polaris::cout << "Identified " << labels.size() << " distinct label(s)" << std::endl;

    return std::make_tuple(pts_to_labels, labels);
}

template POLARIS_API std::tuple<std::vector<std::vector<uint32_t>>, turbo::flat_hash_set<uint32_t>>
parse_formatted_label_file(path label_file);

template POLARIS_API std::tuple<std::vector<std::vector<uint16_t>>, turbo::flat_hash_set<uint16_t>>
parse_formatted_label_file(path label_file);

template POLARIS_API void generate_label_indices<float>(path input_data_path, path final_index_path_prefix,
                                                              label_set all_labels, uint32_t R, uint32_t L, float alpha,
                                                              uint32_t num_threads);
template POLARIS_API void generate_label_indices<uint8_t>(path input_data_path, path final_index_path_prefix,
                                                                label_set all_labels, uint32_t R, uint32_t L,
                                                                float alpha, uint32_t num_threads);
template POLARIS_API void generate_label_indices<int8_t>(path input_data_path, path final_index_path_prefix,
                                                               label_set all_labels, uint32_t R, uint32_t L,
                                                               float alpha, uint32_t num_threads);

template POLARIS_API turbo::flat_hash_map<std::string, std::vector<uint32_t>>
