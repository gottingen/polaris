...
namespace polaris {
    ...
    void generate_label_indices(path input_data_path, path final_index_path_prefix, label_set all_labels, uint32_t R,
        ...
        for (const auto &lbl: all_labels) {
            ...
            polaris::Index<T> index(polaris::MetricType::METRIC_L2, dimension, number_of_label_points,
    ...
    load_label_index_return_values load_label_index(path label_index_path, uint32_t label_number_of_points) {
        ...
        while (bytes_read != index_file_size) {
            ...
            label_index_stream.read((char *) current_node_neighbors.data(),
                                    current_node_num_neighbors * sizeof(uint32_t));
 ...
 */
    ...
    std::tuple<std::vector<std::vector<LabelT>>, turbo::flat_hash_set<LabelT>>
    parse_formatted_label_file(std::string label_file) {
