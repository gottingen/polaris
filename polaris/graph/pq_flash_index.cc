...
PQFlashIndex<T, LabelT>::PQFlashIndex(std::shared_ptr<AlignedFileReader> &fileReader, polaris::Metric m)
    : reader(fileReader), metric(m), _thread_data(nullptr)
{
    polaris::Metric metric_to_invoke = m;
    if (m == polaris::Metric::COSINE || m == polaris::Metric::INNER_PRODUCT)
    {
        if (std::is_floating_point<T>::value)
        {
            polaris::cout << "Since data is floating point, we assume that it has been appropriately pre-processed "
                             "(normalization for cosine, and convert-to-l2 by adding extra dimension for MIPS). So we "
                             "shall invoke an l2 distance function."
                          << std::endl;
            metric_to_invoke = polaris::Metric::L2;
        }
        else
        {
            polaris::cerr << "WARNING: Cannot normalize integral data types."
                                               ...
                                               const bool shuffle)
{
    ...
    polaris::cout << "Level: " << lvl << std::flush;
    polaris::cout << ". #nodes: " << node_list.size() - prev_node_set_size << ", #nodes thus far: " << node_list.size()
                  << std::endl;
    polaris::cout << "done" << std::endl;
}
...
template <typename T, typename LabelT> void PQFlashIndex<T, LabelT>::use_medoids_data_as_centroids()
{
    ...
    polaris::cout << "Loading centroid data from medoids vector data of " << _num_medoids << " medoid(s)" << std::endl;
                                                      ...
                                                      const char *pivots_filepath, const char *compressed_filepath)
{
    ...
    if (collie::filesystem::exists(medoids_file))
    {
        ...
        else
        {
            ...
            polaris::load_aligned_bin<float>(centroids_file, _centroid_data, num_centroids, tmp_dim, aligned_tmp_dim);
    ...
    if (collie::filesystem::exists(norm_file) && metric == polaris::Metric::INNER_PRODUCT)
    {
        uint64_t dumr, dumc;
        float *norm_val;
        polaris::load_bin<float>(norm_file, norm_val, dumr, dumc);
        this->_max_base_norm = norm_val[0];
        polaris::cout << "Setting re-scaling factor of base vectors to " << this->_max_base_norm << std::endl;
        delete[] norm_val;
    }
    polaris::cout << "done.." << std::endl;
    return 0;
                                                 ...
                                                 QueryStats *stats)
{
    ...
    if (beam_width > num_sector_per_nodes * defaults::MAX_N_SECTOR_READS)
        throw PolarisException("Beamwidth can not be higher than defaults::MAX_N_SECTOR_READS", -1, __PRETTY_FUNCTION__, __FILE__,
                           __LINE__);
    ...
    if (metric == polaris::Metric::INNER_PRODUCT || metric == polaris::Metric::COSINE)
    {
        uint64_t inherent_dim = (metric == polaris::Metric::COSINE) ? this->_data_dim : (uint64_t)(this->_data_dim - 1);
    ...
    else
    {
        ...
        {
            throw PolarisException("Cannot find medoid for specified filter.", -1, __PRETTY_FUNCTION__, __FILE__, __LINE__);
        }
    }
...
template <typename T, typename LabelT> polaris::Metric PQFlashIndex<T, LabelT>::get_metric()
{
    return this->metric;
}
