    ...
    IndexConfig build()
    {
        ...
        if (_dynamic_index)
        {
            if (_index_search_params != nullptr && _index_search_params->initial_search_list_size == 0)
                throw PolarisException("Error: please pass initial_search_list_size for building dynamic index.", -1);
        }

        // sanity check
        if (_dynamic_index && _num_frozen_pts == 0)
        {
            polaris::cout << "_num_frozen_pts passed as 0 for dynamic_index. Setting it to 1 for safety." << std::endl;
            _num_frozen_pts = 1;
        }
