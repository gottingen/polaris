    ...
    void FixedChunkPQTable::load_pq_centroid_bin(const char *pq_table_file, size_t num_chunks) {
        ...
        if (nr != 4 && nr != 5) {
            polaris::cout << "Error reading pq_pivots file " << pq_table_file
                          ...
                          << " or " << 5;
            throw polaris::PolarisException("Error reading pq_pivots file at offsets data.", -1, __PRETTY_FUNCTION__,
                                            __FILE__,
                                            __LINE__);
        }

        if (nr == 4) {
            polaris::cout << "Offsets: " << file_offset_data[0] << " " << file_offset_data[1] << " "
                          << file_offset_data[2]
            ...
            throw polaris::PolarisException("Wrong number of offsets in pq_pivots", -1, __PRETTY_FUNCTION__, __FILE__,
                                            __LINE__);
        }
        ...
        if ((nr != NUM_PQ_CENTROIDS)) {
            ...
            throw polaris::PolarisException("Error reading pq_pivots file at pivots data.", -1, __PRETTY_FUNCTION__,
                                            __FILE__,
                                            __LINE__);
        }
        ...
        if ((nr != this->ndims) || (nc != 1)) {
            ...
            throw polaris::PolarisException("Error reading pq_pivots file at centroid data.", -1, __PRETTY_FUNCTION__,
                                            __FILE__,
                                            __LINE__);
        }
        ...
        if (nc != 1 || (nr != num_chunks + 1 && num_chunks != 0)) {
            ...
            throw polaris::PolarisException("Error loading chunk offsets file", -1, __PRETTY_FUNCTION__, __FILE__,
                                            __LINE__);
        }
        ...
        if (collie::filesystem::exists(rotmat_file)) {
            ...
            if (nr != this->ndims || nc != this->ndims) {
                ...
                throw polaris::PolarisException("Error loading rotation matrix file", -1, __PRETTY_FUNCTION__, __FILE__,
                                                __LINE__);
            }
            use_rotation = true;
        }
...
// for the purpose of integration into systems that strictly control resource allocation.
    int generate_pq_pivots_simplified(const float *train_data, size_t num_train, size_t dim, size_t num_pq_chunks,
        ...
        for (size_t i = 0; i < num_pq_chunks; i++) {
            ...
            kmeans::run_lloyds(cur_data, num_train, cur_chunk_size, cur_pivot_data, num_centers, KMEANS_ITERS_FOR_PQ,
                               NULL,
                               closest_center);
...
#pragma omp parallel for schedule(static, 65536)
            ...
            cblas_sgemm(CblasRowMajor, CblasTrans, CblasNoTrans, (MKL_INT) dim, (MKL_INT) dim, (MKL_INT) num_train,
                        1.0f,
    ...
    int generate_pq_data_from_pivots(const std::string &data_file, uint32_t num_centers, uint32_t num_pq_chunks,
        ...
        if (!collie::filesystem::exists(pq_pivots_path)) {
            ...
            if (nr != 4) {
                polaris::cout << "Error reading pq_pivots file " << pq_pivots_path
                              << ". Offsets dont contain correct metadata, # offsets = " << nr << ", but expecting 4.";
                throw polaris::PolarisException("Error reading pq_pivots file at offsets data.", -1,
            ...
            if ((nr != dim) || (nc != 1)) {
                ...
                throw polaris::PolarisException("Error reading pq_pivots file at centroid data.", -1,
                                                __PRETTY_FUNCTION__, __FILE__,
                                                __LINE__);
            }
            ...
            if (nr != (uint64_t) num_pq_chunks + 1 || nc != 1) {
                ...
                throw polaris::PolarisException("Error reading pq_pivots file at chunk offsets.", -1,
                                                __PRETTY_FUNCTION__, __FILE__,
                                                __LINE__);
            }
            ...
            if (use_opq) {
                ...
                if (nr != (uint64_t) dim || nc != dim) {
                    ...
                    throw polaris::PolarisException("Error reading rotation matrix file.", -1, __PRETTY_FUNCTION__,
                                                    __FILE__, __LINE__);
                }
            }
        ...
        for (size_t block = 0; block < num_blocks; block++) {
            ...
            if (use_opq) {
                ...
                // PQ
                cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, (MKL_INT) cur_blk_size, (MKL_INT) dim,
                            (MKL_INT) dim,
...
// This code should only be available in the "build" DLL.
    ...
    void generate_quantized_data(const std::string &data_file_to_use, const std::string &pq_pivots_path,
                                 const std::string &pq_compressed_vectors_path, polaris::MetricType compareMetric,
...
// Instantations of supported templates
    ...
    template POLARIS_API void generate_disk_quantized_data<uint8_t>(
            ...
            const std::string &disk_pq_compressed_vectors_path, polaris::MetricType compareMetric, const double p_val,
            size_t &disk_pq_dims);
