...
void FixedChunkPQTable::load_pq_centroid_bin(const char *pq_table_file, size_t num_chunks)
{
    ...
    polaris::load_bin<size_t>(pq_table_file, file_offset_data, nr, nc);

    bool use_old_filetype = false;

    if (nr != 4 && nr != 5)
    {
        polaris::cout << "Error reading pq_pivots file " << pq_table_file
                      << ". Offsets dont contain correct metadata, # offsets = " << nr << ", but expecting " << 4
                      << " or " << 5;
        throw polaris::PolarisException("Error reading pq_pivots file at offsets data.", -1, __PRETTY_FUNCTION__, __FILE__,
                                    __LINE__);
    }

    if (nr == 4)
    {
        polaris::cout << "Offsets: " << file_offset_data[0] << " " << file_offset_data[1] << " " << file_offset_data[2]
    ...
    {
        throw polaris::PolarisException("Wrong number of offsets in pq_pivots", -1, __PRETTY_FUNCTION__, __FILE__, __LINE__);
    }

    polaris::load_bin<float>(pq_table_file, tables, nr, nc, file_offset_data[0]);

    if ((nr != NUM_PQ_CENTROIDS))
    {
        polaris::cout << "Error reading pq_pivots file " << pq_table_file << ". file_num_centers  = " << nr
                      << " but expecting " << NUM_PQ_CENTROIDS << " centers";
        throw polaris::PolarisException("Error reading pq_pivots file at pivots data.", -1, __PRETTY_FUNCTION__, __FILE__,
                                    __LINE__);
    }

    this->ndims = nc;

    polaris::load_bin<float>(pq_table_file, centroid, nr, nc, file_offset_data[1]);

    if ((nr != this->ndims) || (nc != 1))
    {
        polaris::cerr << "Error reading centroids from pq_pivots file " << pq_table_file << ". file_dim  = " << nr
                      << ", file_cols = " << nc << " but expecting " << this->ndims << " entries in 1 dimension.";
        throw polaris::PolarisException("Error reading pq_pivots file at centroid data.", -1, __PRETTY_FUNCTION__, __FILE__,
                                    __LINE__);
    }
    ...
    polaris::load_bin<uint32_t>(pq_table_file, chunk_offsets, nr, nc, file_offset_data[chunk_offsets_index]);

    if (nc != 1 || (nr != num_chunks + 1 && num_chunks != 0))
    {
        polaris::cerr << "Error loading chunk offsets file. numc: " << nc << " (should be 1). numr: " << nr
                      << " (should be " << num_chunks + 1 << " or 0 if we need to infer)" << std::endl;
        throw polaris::PolarisException("Error loading chunk offsets file", -1, __PRETTY_FUNCTION__, __FILE__, __LINE__);
    }

    this->n_chunks = nr - 1;
    polaris::cout << "Loaded PQ Pivots: #ctrs: " << NUM_PQ_CENTROIDS << ", #dims: " << this->ndims
                  << ", #chunks: " << this->n_chunks << std::endl;

    if (collie::filesystem::exists(rotmat_file))
    {
        polaris::load_bin<float>(rotmat_file, rotmat_tr, nr, nc);
        if (nr != this->ndims || nc != this->ndims)
        {
            polaris::cerr << "Error loading rotation matrix file" << std::endl;
            throw polaris::PolarisException("Error loading rotation matrix file", -1, __PRETTY_FUNCTION__, __FILE__, __LINE__);
        }
        use_rotation = true;
    }
                       ...
                       bool make_zero_mean)
{
    ...
    {
        polaris::cout << " Error: number of chunks more than dimension" << std::endl;
        return -1;
    }
    ...
    if (collie::filesystem::exists(pq_pivots_path))
    {
        ...
        {
            polaris::cout << "PQ pivot file exists. Not generating again" << std::endl;
            return -1;
        }
    }
    ...
    for (size_t i = 0; i < num_pq_chunks; i++)
    {
        ...
        polaris::cout << "Processing chunk " << i << " with dimensions [" << chunk_offsets[i] << ", "
...
#pragma omp parallel for schedule(static, 65536)
    ...
    cumul_bytes[2] = cumul_bytes[1] +
                     polaris::save_bin<float>(pq_pivots_path.c_str(), centroid.get(), (size_t)dim, 1, cumul_bytes[1]);
    ...
    polaris::cout << "Saved pq pivot data to " << pq_pivots_path << " of size " << cumul_bytes[cumul_bytes.size() - 1]
                  << "B." << std::endl;
...
#pragma omp parallel for schedule(static, 65536)
    ...
    cumul_bytes[1] = cumul_bytes[0] + polaris::save_bin<float>(opq_pivots_path.c_str(), full_pivot_data.get(),
                                                               (size_t)num_centers, dim, cumul_bytes[0]);
    cumul_bytes[2] = cumul_bytes[1] +
                     polaris::save_bin<float>(opq_pivots_path.c_str(), centroid.get(), (size_t)dim, 1, cumul_bytes[1]);
    ...
    polaris::cout << "Saved opq pivot data to " << opq_pivots_path << " of size " << cumul_bytes[cumul_bytes.size() - 1]
                  << "B." << std::endl;

    std::string rotmat_path = opq_pivots_path + "_rotation_matrix.bin";
    polaris::save_bin<float>(rotmat_path.c_str(), rotmat_tr.get(), dim, dim);
                                 ...
                                 bool use_opq)
{
    ...
    if (!collie::filesystem::exists(pq_pivots_path))
    {
        ...
        throw polaris::PolarisException("PQ k-means pivot file not found", -1);
    }
    else
    {
        size_t nr, nc;
        ...
        polaris::load_bin<size_t>(pq_pivots_path.c_str(), file_offset_data, nr, nc, 0);

        if (nr != 4)
        {
            polaris::cout << "Error reading pq_pivots file " << pq_pivots_path
                          << ". Offsets dont contain correct metadata, # offsets = " << nr << ", but expecting 4.";
            throw polaris::PolarisException("Error reading pq_pivots file at offsets data.", -1, __PRETTY_FUNCTION__, __FILE__,
                                        __LINE__);
        }

        polaris::load_bin<float>(pq_pivots_path.c_str(), full_pivot_data, nr, nc, file_offset_data[0]);

        if ((nr != num_centers) || (nc != dim))
        {
            polaris::cout << "Error reading pq_pivots file " << pq_pivots_path << ". file_num_centers  = " << nr
        ...
        polaris::load_bin<float>(pq_pivots_path.c_str(), centroid, nr, nc, file_offset_data[1]);

        if ((nr != dim) || (nc != 1))
        {
            polaris::cout << "Error reading pq_pivots file " << pq_pivots_path << ". file_dim  = " << nr
                          << ", file_cols = " << nc << " but expecting " << dim << " entries in 1 dimension.";
            throw polaris::PolarisException("Error reading pq_pivots file at centroid data.", -1, __PRETTY_FUNCTION__, __FILE__,
                                        __LINE__);
        }

        polaris::load_bin<uint32_t>(pq_pivots_path.c_str(), chunk_offsets, nr, nc, file_offset_data[2]);

        if (nr != (uint64_t)num_pq_chunks + 1 || nc != 1)
        {
            polaris::cout << "Error reading pq_pivots file at chunk offsets; file has nr=" << nr << ",nc=" << nc
                          << ", expecting nr=" << num_pq_chunks + 1 << ", nc=1." << std::endl;
            throw polaris::PolarisException("Error reading pq_pivots file at chunk offsets.", -1, __PRETTY_FUNCTION__, __FILE__,
                                        __LINE__);
        }
        ...
        if (use_opq)
        {
            ...
            polaris::load_bin<float>(rotmat_path.c_str(), rotmat_tr, nr, nc);
            if (nr != (uint64_t)dim || nc != dim)
            {
                polaris::cout << "Error reading rotation matrix file." << std::endl;
                throw polaris::PolarisException("Error reading rotation matrix file.", -1, __PRETTY_FUNCTION__, __FILE__, __LINE__);
            }
        }

        polaris::cout << "Loaded PQ pivot information" << std::endl;
    }
    ...
    for (size_t block = 0; block < num_blocks; block++)
    {
        ...
        polaris::convert_types<T, float>(block_data_T.get(), block_data_tmp.get(), cur_blk_size, dim);

        polaris::cout << "Processing points  [" << start_id << ", " << end_id << ").." << std::flush;
...
#pragma omp parallel for schedule(static, 8192)
        ...
        else
        {
            ...
            polaris::convert_types<uint32_t, uint8_t>(block_compressed_base.get(), pVec.get(), cur_blk_size,
                                                      num_pq_chunks);
...
void generate_disk_quantized_data(const std::string &data_file_to_use, const std::string &disk_pq_pivots_path,
                                  const std::string &disk_pq_compressed_vectors_path, polaris::Metric compareMetric,
                                  const double p_val, size_t &disk_pq_dims)
{
    ...
    polaris::cout << "Training data with " << train_size << " samples loaded." << std::endl;
...
void generate_quantized_data(const std::string &data_file_to_use, const std::string &pq_pivots_path,
                             const std::string &pq_compressed_vectors_path, polaris::Metric compareMetric,
                             const double p_val, const size_t num_pq_chunks, const bool use_opq,
                             const std::string &codebook_prefix)
{
    size_t train_size, train_dim;
    float *train_data;
    if (!collie::filesystem::exists(codebook_prefix))
    {
        // instantiates train_data with random sample updates train_size
        gen_random_slice<T>(data_file_to_use.c_str(), p_val, train_data, train_size, train_dim);
        polaris::cout << "Training data with " << train_size << " samples loaded." << std::endl;
    ...
    {
        polaris::cout << "Skip Training with predefined pivots in: " << pq_pivots_path << std::endl;
    }
...
template POLARIS_API int generate_pq_data_from_pivots<int8_t>(const std::string &data_file, uint32_t num_centers,
                                                                    uint32_t num_pq_chunks,
                                                                    const std::string &pq_pivots_path,
                                                                    const std::string &pq_compressed_vectors_path,
                                                                    bool use_opq);
template POLARIS_API int generate_pq_data_from_pivots<uint8_t>(const std::string &data_file, uint32_t num_centers,
                                                                     uint32_t num_pq_chunks,
                                                                     const std::string &pq_pivots_path,
                                                                     const std::string &pq_compressed_vectors_path,
                                                                     bool use_opq);
template POLARIS_API int generate_pq_data_from_pivots<float>(const std::string &data_file, uint32_t num_centers,
                                                                   uint32_t num_pq_chunks,
                                                                   const std::string &pq_pivots_path,
                                                                   const std::string &pq_compressed_vectors_path,
                                                                   bool use_opq);
...
template POLARIS_API void generate_disk_quantized_data<int8_t>(const std::string &data_file_to_use,
                                                                     ...
                                                                     polaris::Metric compareMetric, const double p_val,
                                                                     size_t &disk_pq_dims);
...
template POLARIS_API void generate_disk_quantized_data<uint8_t>(
    ...
    const std::string &disk_pq_compressed_vectors_path, polaris::Metric compareMetric, const double p_val,
    size_t &disk_pq_dims);

template POLARIS_API void generate_disk_quantized_data<float>(const std::string &data_file_to_use,
                                                                    const std::string &disk_pq_pivots_path,
                                                                    const std::string &disk_pq_compressed_vectors_path,
                                                                    polaris::Metric compareMetric, const double p_val,
                                                                    size_t &disk_pq_dims);

template POLARIS_API void generate_quantized_data<int8_t>(const std::string &data_file_to_use,
                                                                const std::string &pq_pivots_path,
                                                                const std::string &pq_compressed_vectors_path,
                                                                polaris::Metric compareMetric, const double p_val,
...
template POLARIS_API void generate_quantized_data<uint8_t>(const std::string &data_file_to_use,
                                                                 const std::string &pq_pivots_path,
                                                                 const std::string &pq_compressed_vectors_path,
                                                                 polaris::Metric compareMetric, const double p_val,
                                                                 const size_t num_pq_chunks, const bool use_opq,
...
template POLARIS_API void generate_quantized_data<float>(const std::string &data_file_to_use,
                                                               const std::string &pq_pivots_path,
                                                               const std::string &pq_compressed_vectors_path,
                                                               polaris::Metric compareMetric, const double p_val,
                                                               const size_t num_pq_chunks, const bool use_opq,
