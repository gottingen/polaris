        ...
        for (uint64_t iter = 0; iter < n_iters; iter++) {
            ...
            while (n_tries <= n_retries) {
                ...
                if (ret != (int64_t) n_ops) {
                    POLARIS_LOG(ERROR)
                    << "io_submit() failed; returned " << ret << ", expected=" << n_ops << ", ernno=" << errno
                    ...
                    if (ret != (int64_t) n_ops) {
                        POLARIS_LOG(ERROR) << "io_getevents() failed; returned " << ret << ", expected=" << n_ops
                                           << ", ernno=" << errno << "=" << ::strerror(-ret) << ", try #"
                                           << n_tries + 1;
                        exit(-1);
                    } else {
                        break;
...
LinuxAlignedFileReader::~LinuxAlignedFileReader() {
    ...
    if (ret == -1) {
        if (errno != EBADF) {
            POLARIS_LOG(ERROR) << "close() not called";
            // close file desc
            ret = ::close(this->file_desc);
            // error checks
            if (ret == -1) {
                POLARIS_LOG(ERROR)<< "close() failed; returned " << ret << ", errno=" << errno << ":" << ::strerror(errno);
            }
        }
    }
}
...
io_context_t &LinuxAlignedFileReader::get_ctx() {
    ...
    if (ctx_map.find(std::this_thread::get_id()) == ctx_map.end()) {
        POLARIS_LOG(ERROR) << "bad thread access; returning -1 as io_context_t";
        return this->bad_ctx;
    } else {
...
void LinuxAlignedFileReader::register_thread() {
    ...
    if (ret != 0) {
        lk.unlock();
        if (ret == -EAGAIN) {
            POLARIS_LOG(ERROR) << "io_setup() failed with EAGAIN: Consider increasing /proc/sys/fs/aio-max-nr";
        } else {
            POLARIS_LOG(ERROR) << "io_setup() failed; returned " << ret << ": " << ::strerror(-ret);
        }
    } else {
        POLARIS_LOG(INFO) << "allocating ctx: " << collie::ptr(ctx) << " to thread-id:" << my_id;
        ctx_map[my_id] = ctx;
    }
    lk.unlock();
}
...
void LinuxAlignedFileReader::deregister_thread() {
    ...
    lk.lock();
    ctx_map.erase(my_id);
    POLARIS_LOG(INFO) << "returned ctx from thread-id:" << my_id;
    lk.unlock();
}
...
void LinuxAlignedFileReader::open(const std::string &fname) {
    ...
    POLARIS_LOG(INFO)<< "Opened file : " << fname;
}
...
void LinuxAlignedFileReader::read(std::vector<AlignedRead> &read_reqs, io_context_t &ctx, bool async) {
    if (async == true) {
        POLARIS_LOG(INFO) << "Async currently not supported in linux.";
    }
    assert(this->file_desc != -1);
