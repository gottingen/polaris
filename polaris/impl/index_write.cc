...
#include <polaris/index/index_additive_quantizer_fast_scan.h>
...
#include <polaris/index/index_ivf_additive_quantizer_fast_scan.h>
#include <polaris/index/index_ivf_flat.h>
#include <polaris/index/index_ivf_independent_quantizer.h>
#include <polaris/index/index_ivf_pq.h>
#include <polaris/index/index_ivf_pq_fast_scan.h>
#include <polaris/index/index_ivf_pqr.h>
#include <polaris/index/index_ivf_spectral_hash.h>
#include <polaris/index/index_lsh.h>
#include <polaris/index/index_lattice.h>
#include <polaris/index/index_nn_descent.h>
#include <polaris/index/index_nsg.h>
...
#include <polaris/index/index_binary_from_float.h>
#include <polaris/index/index_binary_hnsw.h>
#include <polaris/index/index_binary_hash.h>
#include <polaris/index/index_binary_ivf.h>
 ...
 **************************************************************/
    ...
    void write_VectorTransform(const VectorTransform *vt, IOWriter *f) {
        if (const LinearTransform *lt = dynamic_cast<const LinearTransform *>(vt)) {
            ...
            POLARIS_THROW_MSG("cannot serialize this");
        }
        // common fields
        WRITE1(vt->d_in);
        WRITE1(vt->d_out);
    ...
    static void write_NSG(const NSG *nsg, IOWriter *f) {
        ...
        int K = graph->K;
        int N = graph->N;
        POLARIS_THROW_IF_NOT(N == nsg->ntotal);
        POLARIS_THROW_IF_NOT(K == nsg->R);
        POLARIS_THROW_IF_NOT(true == graph->own_fields);
    ...
    void write_index(const Index *idx, IOWriter *f) {
        if (const IndexFlat *idxf = dynamic_cast<const IndexFlat *>(idx)) {
            ...
            POLARIS_THROW_IF_NOT(idxlsqfs || idxrqfs || idxplsqfs || idxprqfs);
            ...
            auto ivprqfs =
                    dynamic_cast<const IndexIVFProductResidualQuantizerFastScan *>(
                            idx);
            POLARIS_THROW_IF_NOT(ivlsqfs || ivrqfs || ivplsqfs || ivprqfs);
            ...
            uint32_t h = dynamic_cast<const IndexHNSWFlat *>(idx) ? fourcc("IHNf")
                                                                  : dynamic_cast<const IndexHNSWPQ *>(idx) ? fourcc(
                            "IHNp")
                                                                                                           : dynamic_cast<const IndexHNSWSQ *>(idx)
                                                                                                             ? fourcc(
                                            "IHNs")
                                                                                                             : dynamic_cast<const IndexHNSW2Level *>(idx)
                                                                                                               ? fourcc(
                                                    "IHN2")
                                                                                                               : 0;
            POLARIS_THROW_IF_NOT(h != 0);
            WRITE1(h);
            ...
            uint32_t h = dynamic_cast<const IndexNSGFlat *>(idx) ? fourcc("INSf")
                                                                 : dynamic_cast<const IndexNSGPQ *>(idx) ? fourcc(
                            "INSp")
                                                                                                         : dynamic_cast<const IndexNSGSQ *>(idx)
                                                                                                           ? fourcc(
                                            "INSs")
                                                                                                           : 0;
            POLARIS_THROW_IF_NOT(h != 0);
            ...
            POLARIS_THROW_IF_NOT(idxnndflat != nullptr);
            ...
            POLARIS_THROW_MSG("don't know how to serialize this type of index");
        }
    }
