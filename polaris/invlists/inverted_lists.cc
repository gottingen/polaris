...
namespace polaris {
    ...
    InvertedListsIterator *InvertedLists::get_iterator(
        ...
        POLARIS_THROW_MSG("get_iterator is not supported");
    }
...
#pragma omp parallel for
    ...
    size_t InvertedLists::copy_subset_to(
            ...
            idx_t a1,
            idx_t a2) const {
        POLARIS_THROW_IF_NOT(nlist == oivf.nlist);
        POLARIS_THROW_IF_NOT(code_size == oivf.code_size);
    ...
    double InvertedLists::imbalance_factor() const {
        ...
        return polaris::imbalance_factor(nlist, hist.data());
    }
 ...
 ******************************************/
    ...
    const uint8_t *HStackInvertedLists::get_single_code(
        ...
        POLARIS_THROW_FMT("offset %zd unknown", offset);
    }
 ...
 ******************************************/
    ...
    namespace {
        ...
        idx_t translate_list_no(const SliceInvertedLists *sil, idx_t list_no) {
            POLARIS_THROW_IF_NOT(list_no >= 0 && list_no < sil->nlist);
...
// find the invlist this number belongs to
        int translate_list_no(const VStackInvertedLists *vil, idx_t list_no) {
            POLARIS_THROW_IF_NOT(list_no >= 0 && list_no < vil->nlist);
    ...
    VStackInvertedLists::VStackInvertedLists(int nil, const InvertedLists **ils_in)
        ...
        for (int i = 0; i < nil; i++) {
            ils.push_back(ils_in[i]);
            POLARIS_THROW_IF_NOT(ils_in[i]->code_size == code_size);
 ...
 ******************************************/
    ...
    MaskedInvertedLists::MaskedInvertedLists(
            ...
            : ReadOnlyInvertedLists(il0->nlist, il0->code_size),
              il0(il0),
              il1(il1) {
        POLARIS_THROW_IF_NOT(il1->nlist == nlist);
        POLARIS_THROW_IF_NOT(il1->code_size == code_size);
    }
 ...
 ******************************************/
    ...
    const {
        POLARIS_THROW_IF_NOT(il0->list_size(list_no) < maxsize);
