 ...
 **********************************************/
    ...
    void OnDiskInvertedLists::update_totsize(size_t new_size) {
        ...
        if (ptr != nullptr) {
            ...
            POLARIS_THROW_IF_NOT_FMT(err == 0, "munmap error: %s", strerror(errno));
        }
        if (totsize == 0) {
...
#define INVALID_OFFSET (size_t)(-1)
    ...
    void OnDiskInvertedLists::update_entries(
        ...
        POLARIS_THROW_IF_NOT(!read_only);
        if (n_entry == 0)
            return;
...
#pragma omp critical
    ...
    void OnDiskInvertedLists::crop_invlists(size_t l0, size_t l1) {
        POLARIS_THROW_IF_NOT(0 <= l0 && l0 <= l1 && l1 <= nlist);
...
/** read from a ArrayInvertedLists into this invertedlist type */
    InvertedLists *OnDiskInvertedListsIOHook::read_ArrayInvertedLists(
        ...
        FileIOReader *reader = dynamic_cast<FileIOReader *>(f);
        POLARIS_THROW_IF_NOT_MSG(reader, "mmap only supported for File objects");
        FILE *fdesc = reader->f;
        ...
        { // do the mmap
            ...
            POLARIS_THROW_IF_NOT_FMT(ret == 0, "fstat failed: %s", strerror(errno));
        ...
        POLARIS_THROW_IF_NOT(o <= ails->totsize);
