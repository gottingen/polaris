...
void HNSW::add_links_starting_from(
    ...
    ::polaris::shrink_neighbor_list(ptdis, link_targets, M);
...
int search_from_candidates(
    ...
    while (candidates.size() > 0) {
        ...
        //     if (!sel || sel->is_member(v1)) {
        //         if (nres < k) {
        //             polaris::maxheap_push(++nres, D, I, d, v1);
        //         } else if (d < D[0]) {
        //             polaris::maxheap_replace_top(nres, D, I, d, v1);
        //         }
        //     }
...
void HNSW::MinimaxHeap::push(storage_idx_t i, float v) {
    if (k == n) {
        ...
        polaris::heap_pop<HC>(k--, dis.data(), ids.data());
    }
    polaris::heap_push<HC>(++k, dis.data(), ids.data(), v, i);
    ++nvalid;
}
