...
void ProductQuantizer::compute_code(const float* x, uint8_t* code) const {
    switch (nbits) {
        case 8:
            polaris::compute_code<PQEncoder8>(*this, x, code);
            break;

        case 16:
            polaris::compute_code<PQEncoder16>(*this, x, code);
            break;

        default:
            polaris::compute_code<PQEncoderGeneric>(*this, x, code);
            break;
    }
}
...
void ProductQuantizer::decode(const uint8_t* code, float* x) const {
    switch (nbits) {
        case 8:
            polaris::decode<PQDecoder8>(*this, code, x);
            break;

        case 16:
            polaris::decode<PQDecoder16>(*this, code, x);
            break;

        default:
            polaris::decode<PQDecoderGeneric>(*this, code, x);
            break;
    }
}
