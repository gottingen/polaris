...
#include <polaris/faiss/utils/approx_topk_hamming/approx_topk_hamming.h>
            ...
            switch (approx_topk_mode) {
                ...
                default: {
                    for (size_t j = j0; j < j1; j++, bs2_ += bytes_per_code) {
                        dis = hc.hamming(bs2_);
                        if (dis < bh_val_[0]) {
                            polaris::maxheap_replace_top<hamdis_t>(
...
void hammings(
    ...
    switch (ncodes) {
        case 8:
            ...
            return;
        case 16:
            polaris::hammings<128>(C64(a), C64(b), na, nb, dis);
            return;
        case 32:
            polaris::hammings<256>(C64(a), C64(b), na, nb, dis);
            return;
        case 64:
            polaris::hammings<512>(C64(a), C64(b), na, nb, dis);
            return;
        default:
            polaris::hammings(C64(a), C64(b), na, nb, ncodes * 8, dis);
            return;
    }
}
...
void hamming_count_thres(
    ...
    switch (ncodes) {
        case 8:
            polaris::hamming_count_thres<64>(
                    C64(bs1), C64(bs2), n1, n2, ht, nptr);
            return;
        case 16:
            polaris::hamming_count_thres<128>(
                    C64(bs1), C64(bs2), n1, n2, ht, nptr);
            return;
        case 32:
            polaris::hamming_count_thres<256>(
                    C64(bs1), C64(bs2), n1, n2, ht, nptr);
            return;
        case 64:
            polaris::hamming_count_thres<512>(
                    C64(bs1), C64(bs2), n1, n2, ht, nptr);
            return;
        default:
...
void crosshamming_count_thres(
    ...
    switch (ncodes) {
        case 8:
            polaris::crosshamming_count_thres<64>(C64(dbs), n, ht, nptr);
            return;
        case 16:
            polaris::crosshamming_count_thres<128>(C64(dbs), n, ht, nptr);
            return;
        case 32:
            polaris::crosshamming_count_thres<256>(C64(dbs), n, ht, nptr);
            return;
        case 64:
...
size_t match_hamming_thres(
    ...
    switch (ncodes) {
        case 8:
            return polaris::match_hamming_thres<64>(
                    C64(bs1), C64(bs2), n1, n2, ht, idx, dis);
        case 16:
            return polaris::match_hamming_thres<128>(
                    C64(bs1), C64(bs2), n1, n2, ht, idx, dis);
        case 32:
            return polaris::match_hamming_thres<256>(
                    C64(bs1), C64(bs2), n1, n2, ht, idx, dis);
        case 64:
