...
#include <polaris/capi/index_scalar_quantizer_c.h>
...
using polaris::Index;
using polaris::IndexIVFScalarQuantizer;
using polaris::IndexScalarQuantizer;
...
int faiss_IndexScalarQuantizer_new_with(
    ...
    try {
        IndexScalarQuantizer* index = new IndexScalarQuantizer(
                d,
                static_cast<polaris::ScalarQuantizer::QuantizerType>(qt),
                static_cast<polaris::MetricType>(metric));
...
int faiss_IndexIVFScalarQuantizer_new_with(
    ...
    try {
        ...
        auto qt_ = static_cast<polaris::ScalarQuantizer::QuantizerType>(qt);
...
int faiss_IndexIVFScalarQuantizer_new_with_metric(
    ...
    try {
        auto q = reinterpret_cast<Index*>(quantizer);
        auto mt = static_cast<polaris::MetricType>(metric);
