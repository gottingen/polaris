...
#include <polaris/capi/ngtq_api.h>
...
NGTQGIndex ngtqg_open_index(const char *index_path, NGTError error) {
    try {
        ...
        auto *index = new NGTQG::NgtqgIndex(index_path_str);
        index->disableLog();
...
void ngtqg_close_index(NGTQGIndex index) {
    ...
    (static_cast<NGTQG::NgtqgIndex *>(index))->close();
    delete static_cast<NGTQG::NgtqgIndex *>(index);
}

static bool ngtqg_search_index_(NGTQG::NgtqgIndex *pindex, NGTQG::SearchQuery *sq, NGTQGQueryParameters &params,
...
template<typename T>
static bool ngtqg_search_index_(NGTQGIndex index, T *query, NGTQGQueryParameters &params, NGTObjectDistances results,
                                NGTError error) {
    ...
    NGTQG::NgtqgIndex *pindex = static_cast<NGTQG::NgtqgIndex *>(index);
...
bool ngtqg_quantize(const char *indexPath, NGTQGQuantizationParameters parameters, NGTError error) {
    try {
        NGTQG::NgtqgIndex::quantize(indexPath, parameters.dimension_of_subvector, parameters.max_number_of_edges,
                                    false);
        return true;
    } catch (std::exception &err) {
        std::stringstream ss;
...
bool qbg_create(const char *indexPath, QBGConstructionParameters *parameters, NGTQGError error) {
    ...
    try {
        ...
        QBG::QbgIndex::create(indexPath, property, globalProperty, localProperty, rotation, objectPath);
...
QBGIndex qbg_open_index(const char *index_path, bool prebuilt, QBGError error) {
    try {
        std::string index_path_str(index_path);
        auto *index = new QBG::QbgIndex(index_path_str, prebuilt);
...
void qbg_close_index(QBGIndex index) {
    if (index == NULL) return;
    (static_cast<QBG::QbgIndex *>(index))->close();
    delete static_cast<QBG::QbgIndex *>(index);
    index = 0;
}
...
bool qbg_save_index(QBGIndex index, QBGError error) {
    ...
    try {
        (static_cast<QBG::QbgIndex *>(index))->save();
    } catch (std::exception &err) {
        std::stringstream ss;
        ss << "Capi : " << __FUNCTION__ << "() : Error: " << err.what();
        operate_error_string_(ss, error);
...
static ObjectID qbg_append_object_(QBGIndex index, T *obj, uint32_t obj_dim, QBGError error) {
    ...
    try {
        auto *pindex = static_cast<QBG::QbgIndex *>(index);
...
uint32_t
qbg_insert_objects_as_uint8(QBGIndex index, uint8_t *objects, uint32_t n_of_objects, ObjectID *ids, NGTError error) {
    return qbg_insert_objects_(index, objects, n_of_objects, ids, error);
}

uint32_t qbg_insert_objects_as_float16(QBGIndex index, NGTFloat16 *objects, uint32_t n_of_objects, ObjectID *ids,
                                       NGTError error) {
...
bool qbg_build_index(const char *index_path, QBGBuildParameters *parameters, QBGError error) {
    ...
    try {
        auto verbose = false;
        QBG::QbgIndex::build(index_path, verbose);
    } catch (polaris::PolarisException &err) {
        std::stringstream ss;
        ss << "Capi : " << __FUNCTION__ << "() : Error: " << err.what();
        operate_error_string_(ss, error);
...
static bool qbg_search_index_(QBG::QbgIndex *pindex, std::vector<float> &query, QBGQueryParameters &param,
                              NGTObjectDistances results) {
...
template<typename T>
static bool
qbg_search_index_(QBGIndex index, T *query, QBGQueryParameters &params, NGTObjectDistances results, QBGError error) {
    if (index == NULL || query == NULL || results == NULL) {
        std::stringstream ss;
        ss << "Capi : " << __FUNCTION__ << "() : parametor error: index = " << index << " query = " << query
           << " results = " << results;
...
template<typename T>
static T *qbg_get_object_(QBGIndex index, ObjectID id, QBGError error) {
    if (index == NULL) {
        std::stringstream ss;
