...
void check_compatible_for_merge(const Index* index0, const Index* index1) {
    const polaris::IndexPreTransform* pt0 =
            dynamic_cast<const polaris::IndexPreTransform*>(index0);
    ...
    const polaris::IndexIVF* ivf0 = dynamic_cast<const polaris::IndexIVF*>(index0);
    if (ivf0) {
...
void merge_into(polaris::Index* index0, polaris::Index* index1, bool shift_ids) {
...
void search_centroid(
    ...
    if (auto index_pre = dynamic_cast<polaris::IndexPreTransform*>(index)) {
        x = index_pre->apply_chain(n, x);
        del.reset((float*)x);
        index = index_pre->index;
    }
    polaris::IndexIVF* index_ivf = dynamic_cast<polaris::IndexIVF*>(index);
    assert(index_ivf);
