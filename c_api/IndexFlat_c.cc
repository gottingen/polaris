...
using polaris::Index;
using polaris::IndexFlat;
using polaris::IndexFlat1D;
using polaris::IndexFlatIP;
using polaris::IndexFlatL2;
using polaris::IndexRefineFlat;
...
int faiss_IndexFlat_new_with(
    ...
    try {
        IndexFlat* index =
                new IndexFlat(d, static_cast<polaris::MetricType>(metric));
...
int faiss_IndexRefineFlat_new(
    ...
    try {
        IndexRefineFlat* index = new IndexRefineFlat(
                reinterpret_cast<polaris::Index*>(base_index));
