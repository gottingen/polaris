...
namespace polaris {
    template<typename T>
    void compare_l1_test(uint32_t dim , uint32_t num = 1000) {
        ...
        for (int k = 0; k < num; ++k) {
            ...
            float simple_result = primitive::compare_simple_l1(a.data(), b.data(), dim);
            //float simplef_result = primitive::comparef_l1(a.data(), b.data(), dim);
            float result = polaris::ComparatorL1<T>(dim)(ArrayView(a.data()), ArrayView(b.data()));
            ...
            // presicion is 0.05
            REQUIRE_LT(diff, 0.05f);
        }
    }

    TEST_CASE_TEMPLATE_DEFINE("distance l1", T, l1_dis_test) {
        compare_l1_test<T>(128u);
    }
