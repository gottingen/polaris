...
namespace polaris {
    ...
    void compare_l2_test(uint32_t dim , uint32_t num = 1000) {
        ...
        for (int k = 0; k < num; ++k) {
            ...
            float simple_result = primitive::compare_simple_cosine_similarity(a.data(), b.data(), dim);
            float result = polaris::ComparatorCosineSimilarity<T>(dim)(ArrayView(a.data()), ArrayView(b.data()));
